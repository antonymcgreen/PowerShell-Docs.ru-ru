...
ms.locfileid: "62057360"
---
# <a name="powershellget-cmdlets-for-script-management"></a><span data-ttu-id="08665-102">Командлеты PowerShellGet для управления сценариями</span><span class="sxs-lookup"><span data-stu-id="08665-102">PowerShellGet Cmdlets for Script Management</span></span>

## <a name="find-script-cmdlet"></a><span data-ttu-id="08665-103">Командлет Find-Script</span><span class="sxs-lookup"><span data-stu-id="08665-103">Find-Script cmdlet</span></span>
<span data-ttu-id="08665-104">Командлет Find-Script позволяет обнаруживать файлы сценариев по разным условиям, таким как имя, тег, фильтр, имя команды, диапазон версий, точная версия, все версии, включая зависимости, в отдельных или всех зарегистрированных репозиториях.</span><span class="sxs-lookup"><span data-stu-id="08665-104">Find-Script cmdlet lets you to discover the script files with different search criteria like name, tag, filter, command name, version range, exact version, all versions, including its dependencies and from specific or all registered repositories.</span></span>

<span data-ttu-id="08665-105">Пример использования:</span><span class="sxs-lookup"><span data-stu-id="08665-105">Example usage:</span></span>
```powershell
...
## <a name="save-script-cmdlet"></a><span data-ttu-id="08665-106">Командлет Save-Script</span><span class="sxs-lookup"><span data-stu-id="08665-106">Save-Script cmdlet</span></span>
<span data-ttu-id="08665-107">Командлет Save-Script позволяет просмотреть файл сценария, сохранив его в указанном расположении.</span><span class="sxs-lookup"><span data-stu-id="08665-107">Save-Script cmdlet lets you to review the script file by saving it to a specified location.</span></span>
```powershell
...
## <a name="install-script-and-get-installedscript-cmdlets"></a><span data-ttu-id="08665-108">Командлеты Install-Script и Get-InstalledScript</span><span class="sxs-lookup"><span data-stu-id="08665-108">Install-Script and Get-InstalledScript cmdlets</span></span>
<span data-ttu-id="08665-109">Командлет Install-Script позволяет установить определенный файл сценария вместе с его зависимости в указанной области.</span><span class="sxs-lookup"><span data-stu-id="08665-109">Install-Script cmdlet lets you to install a specific script file along with its dependencies to the specified scope.</span></span> <span data-ttu-id="08665-110">По умолчанию сценарии устанавливаются в область AllUsers.</span><span class="sxs-lookup"><span data-stu-id="08665-110">By default, scripts are installed to the AllUsers scope.</span></span> <span data-ttu-id="08665-111">Командлет Get-InstalledScript позволяет получить список файлов сценариев, которые были установлены с помощью командлета Install-Script.</span><span class="sxs-lookup"><span data-stu-id="08665-111">Get-InstalledScript cmdlet lets you to get the list of script files which were installed using Install-Script cmdlet.</span></span>

<span data-ttu-id="08665-112">Примечание. Чтобы разрешить поиск сценариев и управление ими после установки, сценарий командлет Install-Script создает папку по умолчанию для хранения сценариев в каталоге $home\Documents\WindowsPowerShell\Scripts и добавляет ее в свою переменную среды PATH.</span><span class="sxs-lookup"><span data-stu-id="08665-112">Use note: To allow management and locating of scripts once they are installed, Install-script will create a default folder for storing scripts at $home\Documents\WindowsPowerShell\Scripts, and add that folder to your PATH environment.</span></span> <span data-ttu-id="08665-113">Если изменение пути сопряжено с трудностями, используйте вместо Install-Script командлет Save-Script.</span><span class="sxs-lookup"><span data-stu-id="08665-113">If modifying the path is a concern, use Save-Script instead of Install-Script.</span></span> <span data-ttu-id="08665-114">Get-InstalledScripts и Uninstall-Script работают только со сценариями, размещенными в системе с помощью Install-Script.</span><span class="sxs-lookup"><span data-stu-id="08665-114">Get-InstalledScripts and Uninstall-Script can only work with scripts placed on the system using Install-Script.</span></span>
```powershell
...
<span data-ttu-id="08665-115">Для его получения можно также использовать Get-Command –Name &lt;InstalledScriptFileName&gt;.</span><span class="sxs-lookup"><span data-stu-id="08665-115">You can also use Get-Command –Name &lt;InstalledScriptFileName&gt; to get it.</span></span> <span data-ttu-id="08665-116">При первом использовании указанной области в переменную среды PATH добавляются два расположения установки.</span><span class="sxs-lookup"><span data-stu-id="08665-116">Two install locations are added to the PATH environment variable on first use of a specified scope.</span></span>
```powershell
...
## <a name="update-script-cmdlet"></a><span data-ttu-id="08665-117">Командлет Update-Script</span><span class="sxs-lookup"><span data-stu-id="08665-117">Update-Script cmdlet</span></span>
<span data-ttu-id="08665-118">Командлет Update-Script позволяет выполнить обновление на месте для файлов сценариев, которые были установлены с помощью командлета Install-Script.</span><span class="sxs-lookup"><span data-stu-id="08665-118">Update-Script cmdlet lets you to do in-place update of the script files which were installed using Install-Script cmdlet.</span></span>
```powershell
...
## <a name="uninstall-script-cmdlet"></a><span data-ttu-id="08665-119">Командлет Uninstall-Script</span><span class="sxs-lookup"><span data-stu-id="08665-119">Uninstall-Script cmdlet</span></span>
```powershell
...
## <a name="new-scriptfileinfo-and-test-scriptfileinfo-cmdlets"></a><span data-ttu-id="08665-120">Командлеты New-ScriptFileInfo и Test-ScriptFileInfo</span><span class="sxs-lookup"><span data-stu-id="08665-120">New-ScriptFileInfo and Test-ScriptFileInfo cmdlets</span></span>
<span data-ttu-id="08665-121">Командлет New-ScriptFileInfo позволяет создать файл сценария с такими метаданными, как Version, Guid, Author, Description и т. д. Командлет Test-ScriptFileInfo позволяет проверить и получить метаданные файла сценария.</span><span class="sxs-lookup"><span data-stu-id="08665-121">New-ScriptFileInfo cmdlet lets you to create a new script file with metadata like Version, Guid, Author, and Description, etc. Test-ScriptFileInfo cmdlet lets you to validate and get the script file metadata.</span></span>
```powershell
...
## <a name="update-scriptfileinfo-cmdlet"></a><span data-ttu-id="08665-122">Командлет Update-ScriptFileInfo</span><span class="sxs-lookup"><span data-stu-id="08665-122">Update-ScriptFileInfo cmdlet</span></span>
<span data-ttu-id="08665-123">Командлет Update-ScriptFileInfo позволяет обновить метаданные существующего файла сценария.</span><span class="sxs-lookup"><span data-stu-id="08665-123">Update-ScriptFileInfo cmdlet lets you to update the existing script file metadata.</span></span>
```powershell
...
## <a name="register-psrepository-and-set-psrepository-cmdlets-with-script-sharing-support"></a><span data-ttu-id="08665-124">Командлеты Register-PSRepository и Set-PSRepository с поддержкой совместного использования сценариев</span><span class="sxs-lookup"><span data-stu-id="08665-124">Register-PSRepository and Set-PSRepository cmdlets with script sharing support</span></span>
<span data-ttu-id="08665-125">Используйте командлеты Register-PSRepository/Set-PSRepository для добавления **ScriptSourceLocation** и **ScriptPublishLocation** в PSRepository.</span><span class="sxs-lookup"><span data-stu-id="08665-125">Use Register-PSRepository/Set-PSRepository cmdlets to add the **ScriptSourceLocation** and **ScriptPublishLocation** to the PSRepository.</span></span>
```powershell
...
## <a name="publish-script-cmdlet"></a><span data-ttu-id="08665-126">Публикация сценарным командлетом Publish-Script</span><span class="sxs-lookup"><span data-stu-id="08665-126">Publish-Script cmdlet</span></span>
<span data-ttu-id="08665-127">Командлет Publish-Script позволяет опубликовать файл сценария с допустимым метаданными, такими как Version, Guid, Author, Description и т. д.</span><span class="sxs-lookup"><span data-stu-id="08665-127">Publish-Script cmdlet lets you to publish your script file with valid metadata like Version, Guid, Author, and Description, etc.</span></span>
```powershell
