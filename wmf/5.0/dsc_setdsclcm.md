...
ms.locfileid: "62057956"
---
# <a name="set-dsclocalconfigurationmanager-cmdlet-supports--force-parameter"></a><span data-ttu-id="51d78-102">Командлет Set-DscLocalConfigurationManager поддерживает параметр -force</span><span class="sxs-lookup"><span data-stu-id="51d78-102">Set-DscLocalConfigurationManager cmdlet supports -force parameter</span></span>

<span data-ttu-id="51d78-103">Для командлета Set-DscLocalConfigurationManager добавлена поддержка нового параметра.</span><span class="sxs-lookup"><span data-stu-id="51d78-103">We have added a support for new parameter to Set-DscLocalConfigurationManager cmdlet.</span></span> <span data-ttu-id="51d78-104">Это позволит пользователю детерминировано выполнить сброс метаконфигурации на компьютере, когда в фоновом режиме выполняются другие операции, такие как проверка согласованности, так как это приведет к их остановке.</span><span class="sxs-lookup"><span data-stu-id="51d78-104">This will allow the user to reset meta configuration on machine deterministically when other operations like consistency check are running in background as it will cause all running operations to be stopped.</span></span>

<span data-ttu-id="51d78-105">При попытке установить метаконфигурацию без параметра –Force процедура выглядит следующим образом:</span><span class="sxs-lookup"><span data-stu-id="51d78-105">The experience looks like this when trying to set meta configuration without –Force parameter.</span></span>
```powershell
...
<span data-ttu-id="51d78-106">При использовании –force метаконфигурация в системе успешно обновляется путем отмены выполнения текущей операции на компьютере.</span><span class="sxs-lookup"><span data-stu-id="51d78-106">When we use –force it successfully updates the meta configuration on system by canceling the current running operation on the machine.</span></span>
```powershell
