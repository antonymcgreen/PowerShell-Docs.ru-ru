...
Сводка: Данный документ предназначен для включения в процесс и расширяемости для помощи инженерами, которые подготавливаются для работы решения. Информация в документе содержит в себе рекомендации, изложенные клиентами и затем проверенные рабочей группой по продукту, что позволяет гарантировать их перспективность и стабильность.
...
[режимах конфигурации Push и Pull](/powershell/dsc/pullServer).
...
Помимо установки последнего содержимого из центра обновления Windows, существует скачать два компонента, рекомендуемых для развертывания опрашивающего сервера DSC: Последняя версия Windows Management Framework и модуль DSC для автоматизированной подготовки опрашивающего сервера.
...
Процессор: 1,4 ГГц 64-разрядный процессор памяти: 512 МБ свободного места на жестком диске. Сеть 32 ГБ: Адаптер Gigabit Ethernet
...
Примечание.  В настоящее время `xPSDesiredStateConfiguation` модуля DSC требует, чтобы быть языкового стандарта EN-US сервер.
