...
Самый простой способ настроить опрашиваемый веб-сервер — использовать ресурс **xDscWebService**, включенный в модуль **xPSDesiredStateConfiguration**.
...
1. Вызовите командлет [Install-Module](/powershell/module/PowershellGet/Install-Module), чтобы установить модуль **xPSDesiredStateConfiguration**.
   > [!NOTE]
   > **Install-Module** включается в **PowerShellGet** модуль, который включается в PowerShell 5.0. Вы можете скачать модуль **PowerShellGet**для PowerShell 3.0 и 4.0 в разделе [Предварительная версия модулей PackageManagement PowerShell](https://www.microsoft.com/en-us/download/details.aspx?id=49186).
...
3. Установите сертификат на узле, который должен стать сервером DSC на включение внесенных изменений в расположении по умолчанию, который должен быть `CERT:\LocalMachine\My`.
...
5. В PowerShell ISE запустите (F5) следующий сценарий конфигурации (включен в папке примеров **xPSDesiredStateConfiguration** модуль в виде `Sample_xDscWebServiceRegistration.ps1`). Этот сценарий настраивает опрашиваемый сервер.
...
6. Запустите конфигурацию, передав отпечаток SSL-сертификата в виде параметра **certificateThumbPrint** и ключ регистрации GUID в виде параметра **RegistrationKey**:
...
Для настройки проверки подлинности узла опрашиваемом сервере необходимо поместить ключ регистрации в метаконфигурацию всех целевых узлов, которые будут регистрироваться на этом опрашиваемом сервере. Обратите внимание, что **RegistrationKey** в метаконфигурации ниже удаляется после успешной регистрации целевого компьютера, а значение должно соответствовать значению, хранящемуся в `RegistrationKeys.txt` файл на опрашивающем сервере (" 140a952b-b9d6-406b-b416-e0f759c9c0e4 "в этом примере). Значение ключа регистрации должно оставаться конфиденциальным, так как с ним любой целевой компьютер сможет зарегистрироваться на опрашиваемом сервере.
...
> [!NOTE]
> **ReportServerWeb** позволяет данные, отправляемые на опрашивающий сервер отчетов.
...
Перед упаковкой для опрашивающего сервера удалите **{версия_модуля}** папки, чтобы путь стал `{Module Folder}\DscResources\{DSC Resource Folder}\`.
...
1. Модуль, который помогает упаковывать модули ресурсов и конфигурационные файлы DSC для использования на опрашивающем сервере.
...
- [Настройка опрашивающего клиента DSC с помощью имен конфигураций](pullClientConfigNames.md)
