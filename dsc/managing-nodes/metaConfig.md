...
> [!NOTE]
> Начинает LCM **ConfigurationModeFrequencyMins** на основе цикла:
>
> - С помощью применяется новый метаконфигурацию `Set-DscLocalConfigurationManager`
> - Перезагрузки компьютера
>
> Любое условие, где процесс таймера возникает сбой, который обнаруживается в течение 30 секунд, цикл будет перезапущен.
> Параллельная операция может отложить цикла при запуске, если частота настроенных цикла, превышает длительность этой операции таймера не запустится.
>
> Пример, метаконфигурацию настроен с частотой 15-минутный по запросу, а по запросу в T1.  Узел не завершается по обеспечению 16 минут.  Первый цикл 15-минутный игнорируется, и далее по запросу будет происходить при T1 + 15 + 15.
