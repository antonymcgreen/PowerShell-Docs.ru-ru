...
1. [Установка Windows PowerShell Web Access](#install-windows-powershell-web-access-using-powershell-cmdlets)
...
![Схема Windows PowerShell Web Access](media/install-and-use-windows-powershell-web-access/Windows-PowerShell-Web-Access-diagram.jpg)
...
1. Введите следующую команду и нажмите клавишу **ВВОД**, где *computer_name* представляет имя удаленного компьютера, на который требуется установить Windows PowerShell Web Access, если это применимо. Параметр `-Restart` автоматически при необходимости перезапускает целевые серверы.
   ...
   Чтобы установить роли и компоненты на автономный виртуальный жесткий диск, необходимо добавить оба параметра, `-ComputerName` и `-VHD`. В параметре `-ComputerName` содержится имя сервера, на котором требуется подключить виртуальный жесткий диск, а в параметре `-VHD` — путь к VHD-файлу на указанном сервере.
...
2. Необязательный шаг для ограничения пользовательского доступа путем использования конфигураций сеансов: Убедитесь, что конфигурации сеансов, которые требуется использовать в правилах, уже существуют. В противном случае выполните инструкции по созданию конфигураций сеансов, приведенные в статье [about_Session_Configuration_Files](/powershell/module/microsoft.powershell.core/about/about_session_configurations).
...
## <a name="custom-deployment"></a>Настраиваемое развертывание
...
Этот раздел содержит инструкции по установке веб-приложения Windows PowerShell Web Access в подкаталог, а не в корневой каталог веб-сайта. Данная процедура, выполняемая через графический пользовательский интерфейс, эквивалентна действиям, выполняемым с помощью командлета `Install-PswaWebApplication`. В раздел также включены инструкции по использованию диспетчера служб IIS для настройки шлюза Windows PowerShell Web Access в качестве корневого веб-сайта.
...
1. Когда новый веб-сайт выбран в области дерева диспетчера служб IIS, в области **Действия** щелкните **Запуск**, чтобы запустить веб-сайт.
...
1. Необязательный шаг для ограничения пользовательского доступа путем использования конфигураций сеансов:
...
1. В области **Действия** выполните одно из следующих действий. Дополнительные сведения о настройке сертификатов сервера в IIS см. в статье [Настройка сертификатов сервера в службах IIS 7](/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc732230(v=ws.10)).
   ...
   - Щелкните **Создать запрос сертификата**, чтобы запросить сертификат из центра сертификации, например из [VeriSign](https://www.verisign.com/), [Thawte](https://www.thawte.com/) или [GeoTrust](https://www.geotrust.com/). Общее имя сертификата должно совпадать с заголовком узла в запросе.
