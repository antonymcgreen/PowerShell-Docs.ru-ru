...
## <a name="improved-backwards-compatibility-with-windows-powershell"></a><span data-ttu-id="3cac4-146">Улучшенная обратная совместимость с Windows PowerShell</span><span class="sxs-lookup"><span data-stu-id="3cac4-146">Improved backwards compatibility with Windows PowerShell</span></span>
...
- <span data-ttu-id="3cac4-167">Выражение `<if-true>` выполняется, если выражение `<condition>` имеет значение true.</span><span class="sxs-lookup"><span data-stu-id="3cac4-167">The `<if-true>` expression is executed if the `<condition>` expression is true</span></span>
...
<span data-ttu-id="3cac4-180">В этом примере первая команда завершается успешно и выполняется вторая команда:</span><span class="sxs-lookup"><span data-stu-id="3cac4-180">Here, the first command succeeds and the second command is executed:</span></span>
...
<span data-ttu-id="3cac4-195">В следующем примере правый операнд не вычисляется:</span><span class="sxs-lookup"><span data-stu-id="3cac4-195">In the following example, the right-hand operand is not evaluated:</span></span>
...
- <span data-ttu-id="3cac4-240">**LTS** — выводятся уведомления об обновлениях только для общедоступных выпусков ветви Long Term Servicing Branch (LTS).</span><span class="sxs-lookup"><span data-stu-id="3cac4-240">**LTS** only notifies of updates to long-term-servicing (LTS) GA releases</span></span>
...
<span data-ttu-id="3cac4-252">Эта команда вызывает метод **Set** ресурса с именем **WindowsProcess** и предоставляет обязательные свойства **Path** и **Arguments** для запуска указанного процесса Windows.</span><span class="sxs-lookup"><span data-stu-id="3cac4-252">This command invokes the **Set** method of a resource named **WindowsProcess** and provides the mandatory **Path** and **Arguments** properties to start the specified Windows process.</span></span>
...
- <span data-ttu-id="3cac4-263">Улучшение работы API-интерфейсов точек останова в удаленных сценариях (№ 11312)</span><span class="sxs-lookup"><span data-stu-id="3cac4-263">Improvements in breakpoint APIs for remote scenarios (#11312)</span></span>
...
- <span data-ttu-id="3cac4-282">Поддержка отрицательных чисел в операторе -split (8960) (Спасибо, @ece-jacob-scott!)</span><span class="sxs-lookup"><span data-stu-id="3cac4-282">Support negative numbers in -split operator (#8960) (Thanks @ece-jacob-scott!)</span></span>
...
- <span data-ttu-id="3cac4-321">Исправление GetCorrectCasedPath(): перед вызовом First() сначала выполняется проверка того, возвращаются ли какие-либо записи файловой системы (№ 10930)</span><span class="sxs-lookup"><span data-stu-id="3cac4-321">Fix GetCorrectCasedPath() to first check if any system file entries is returned before calling First() (#10930)</span></span>
...
- <span data-ttu-id="3cac4-339">Поддержка заполнения нажатием клавиши TAB при присвоении значения переменной, являющейся перечислением или переменной с ограничением типа (№ 10646)</span><span class="sxs-lookup"><span data-stu-id="3cac4-339">Enable tab completion for variable assignment that is enum or type constrained (#10646)</span></span>
...
- <span data-ttu-id="3cac4-357">Исправлено исключение пустой ссылки при обработке входных данных ForEach-Object -Parallel (№ 10577)</span><span class="sxs-lookup"><span data-stu-id="3cac4-357">Fix null reference exception in ForEach-Object -Parallel input processing (#10577)</span></span>
...
- <span data-ttu-id="3cac4-371">Не используйте замыкание в Parser.SaveError (№ 11006)</span><span class="sxs-lookup"><span data-stu-id="3cac4-371">Avoid using closure in Parser.SaveError (#11006)</span></span>
...
- <span data-ttu-id="3cac4-388">Восстановлен интерфейс API отладки, который перестал работать из-за исправления № 10338 (№ 10808)</span><span class="sxs-lookup"><span data-stu-id="3cac4-388">Reinstate debugging API lost in #10338 (#10808)</span></span>
- <span data-ttu-id="3cac4-389">Удалена ссылка на WorkflowJobSourceAdapter, которая больше не используется (№ 10326) (выражаем благодарность @KirkMunro)</span><span class="sxs-lookup"><span data-stu-id="3cac4-389">Remove WorkflowJobSourceAdapter reference that is no longer used (#10326) (Thanks @KirkMunro!)</span></span>
- <span data-ttu-id="3cac4-390">Очистка кода списка переходов для интерфейсов COM путем внесения исправлений в атрибуты PreserveSig (№ 9899) (выражаем благодарность @weltkante)</span><span class="sxs-lookup"><span data-stu-id="3cac4-390">Cleanup COM interfaces in jump list code by fixing PreserveSig attributes (#9899) (Thanks @weltkante!)</span></span>
- <span data-ttu-id="3cac4-391">Добавлен комментарий, описывающий, почему -ia не является псевдонимом для общего параметра -InformationAction (№ 10703) (выражаем благодарность @KirkMunro)</span><span class="sxs-lookup"><span data-stu-id="3cac4-391">Add a comment describing why -ia is not the alias for -InformationAction common parameter (#10703) (Thanks @KirkMunro!)</span></span>
- <span data-ttu-id="3cac4-392">Файл InvokeCommandCmdlet.cs переименован в InvokeExpressionCommand.cs (№ 10659) (выражаем благодарность @kilasuit)</span><span class="sxs-lookup"><span data-stu-id="3cac4-392">Rename InvokeCommandCmdlet.cs to InvokeExpressionCommand.cs (#10659) (Thanks @kilasuit!)</span></span>
- <span data-ttu-id="3cac4-393">Незначительная очистка кода, связанного с уведомлениями об обновлениях (№ 10698)</span><span class="sxs-lookup"><span data-stu-id="3cac4-393">Add minor code cleanups related to update notifications (#10698)</span></span>
- <span data-ttu-id="3cac4-394">Удалена нерекомендуемая логика рабочего процесса из скриптов настройки удаленного взаимодействия (№ 10320) (выражаем благодарность @KirkMunro)</span><span class="sxs-lookup"><span data-stu-id="3cac4-394">Remove deprecated workflow logic from the remoting setup scripts (#10320) (Thanks @KirkMunro!)</span></span>
- <span data-ttu-id="3cac4-395">Обновление формата справки с целью использования правильного регистра символов (№ 10678) (выражаем благодарность @tnieto88)</span><span class="sxs-lookup"><span data-stu-id="3cac4-395">Update help format to use proper case (#10678) (Thanks @tnieto88!)</span></span>
- <span data-ttu-id="3cac4-396">Исправление проблем со стилем CodeFactor в фиксациях за последний месяц (№ 10591) (выражаем благодарность @iSazonov)</span><span class="sxs-lookup"><span data-stu-id="3cac4-396">Clean up CodeFactor style issues coming in commits for the last month (#10591) (Thanks @iSazonov!)</span></span>
- <span data-ttu-id="3cac4-397">Исправлена опечатка в описании экспериментальной возможности PSTernaryOperator (№ 10586) (выражаем благодарность @bergmeister)</span><span class="sxs-lookup"><span data-stu-id="3cac4-397">Fix typo in description of PSTernaryOperator experimental feature (#10586) (Thanks @bergmeister!)</span></span>
- <span data-ttu-id="3cac4-398">Значение перечисления ActionPreference.Suspend переведено в неподдерживаемое зарезервированное состояние; устранено ограничение на использование ActionPreference.Ignore в привилегированных переменных (№ 10317) (выражаем благодарность @KirkMunro)</span><span class="sxs-lookup"><span data-stu-id="3cac4-398">Convert ActionPreference.Suspend enumeration value into a non-supported, reserved state, and remove restriction on using ActionPreference.Ignore in preference variables (#10317) (Thanks @KirkMunro!)</span></span>
- <span data-ttu-id="3cac4-399">Класс ArrayList заменен на List\<T> для повышения удобочитаемости и надежности кода без изменения функциональности (№ 10333) (выражаем благодарность @iSazonov)</span><span class="sxs-lookup"><span data-stu-id="3cac4-399">Replace ArrayList with List\<T> to get more readable and reliable code without changing functionality (#10333) (Thanks @iSazonov!)</span></span>
- <span data-ttu-id="3cac4-400">Исправления стиля кода в TestConnectionCommand (№ 10439) (выражаем благодарность @vexx32)</span><span class="sxs-lookup"><span data-stu-id="3cac4-400">Make code style fixes to TestConnectionCommand (#10439) (Thanks @vexx32!)</span></span>
- <span data-ttu-id="3cac4-401">Очистка AutomationEngine и удаление лишнего вызова метода SetSessionStateDrive (№ 10416) (выражаем благодарность @iSazonov)</span><span class="sxs-lookup"><span data-stu-id="3cac4-401">Cleanup AutomationEngine and remove extra SetSessionStateDrive method call (#10416) (Thanks @iSazonov!)</span></span>
- <span data-ttu-id="3cac4-402">Переименование атрибута ParameterSetName обратно в Delimiter для командлетов ConvertTo-Csv и ConvertFrom-Csv (№ 10425)</span><span class="sxs-lookup"><span data-stu-id="3cac4-402">Rename default ParameterSetName back to Delimiter for ConvertTo-Csv and ConvertFrom-Csv (#10425)</span></span>

### <a name="tools"></a><span data-ttu-id="3cac4-403">Инструменты</span><span class="sxs-lookup"><span data-stu-id="3cac4-403">Tools</span></span>

- <span data-ttu-id="3cac4-404">Добавлено значение по умолчанию для свойства SDKToUse для обеспечения сборки в Visual Studio (№ 11085)</span><span class="sxs-lookup"><span data-stu-id="3cac4-404">Add default setting for the SDKToUse property so that it builds in VS (#11085)</span></span>
- <span data-ttu-id="3cac4-405">Install-Powershell.ps1: добавлен параметр для использования установки MSI (№ 10921) (выражаем благодарность @MJECloud)</span><span class="sxs-lookup"><span data-stu-id="3cac4-405">Install-Powershell.ps1: Add parameter to use MSI installation (#10921) (Thanks @MJECloud!)</span></span>
- <span data-ttu-id="3cac4-406">Добавлены базовые примеры для install-powershell.ps1 (№ 10914) (выражаем благодарность @kilasuit)</span><span class="sxs-lookup"><span data-stu-id="3cac4-406">Add basic examples for install-powershell.ps1 (#10914) (Thanks @kilasuit!)</span></span>
- <span data-ttu-id="3cac4-407">Install-PowerShellRemoting.ps1 теперь обрабатывает пустую строку в параметре PowerShellHome (№ 10526) (выражаем благодарность @Orca88)</span><span class="sxs-lookup"><span data-stu-id="3cac4-407">Make Install-PowerShellRemoting.ps1 handle empty string in PowerShellHome parameter (#10526) (Thanks @Orca88!)</span></span>
- <span data-ttu-id="3cac4-408">Замена /etc/lsb-release на /etc/os-release в install-powershell.sh (№ 10773) (выражаем благодарность @Himura2la)</span><span class="sxs-lookup"><span data-stu-id="3cac4-408">Switch from /etc/lsb-release to /etc/os-release in install-powershell.sh (#10773) (Thanks @Himura2la!)</span></span>
- <span data-ttu-id="3cac4-409">Проверка pwsh.exe и pwsh в ежедневной версии в Windows (№ 10738) (выражаем благодарность @centreboard)</span><span class="sxs-lookup"><span data-stu-id="3cac4-409">Check pwsh.exe and pwsh in daily version on Windows (#10738) (Thanks @centreboard!)</span></span>
- <span data-ttu-id="3cac4-410">Удален ненужный отвод в installpsh-osx.sh (№ 10752)</span><span class="sxs-lookup"><span data-stu-id="3cac4-410">Remove unneeded tap in installpsh-osx.sh (#10752)</span></span>
- <span data-ttu-id="3cac4-411">Обновление файла install-powershell.ps1 для проверки уже установленной ежедневной сборки (№ 10489)</span><span class="sxs-lookup"><span data-stu-id="3cac4-411">Update install-powershell.ps1 to check for already installed daily build (#10489)</span></span>

### <a name="tests"></a><span data-ttu-id="3cac4-412">Тесты</span><span class="sxs-lookup"><span data-stu-id="3cac4-412">Tests</span></span>

- <span data-ttu-id="3cac4-413">Перевод теста DSC с недостоверным результатом в состояние ожидания (№ 11131)</span><span class="sxs-lookup"><span data-stu-id="3cac4-413">Make unreliable DSC test pending (#11131)</span></span>
- <span data-ttu-id="3cac4-414">Исправление теста строковых данных для правильной проверки ключей хэш-таблиц (№ 10810)</span><span class="sxs-lookup"><span data-stu-id="3cac4-414">Fix stringdata test to correctly validate keys of hashtables (#10810)</span></span>
- <span data-ttu-id="3cac4-415">Выгрузка тестовых модулей (№ 11061) (выражаем благодарность @iSazonov)</span><span class="sxs-lookup"><span data-stu-id="3cac4-415">Unload test modules (#11061) (Thanks @iSazonov!)</span></span>
- <span data-ttu-id="3cac4-416">Увеличен интервал между повторными попытками проверки URL-адреса (№ 11015)</span><span class="sxs-lookup"><span data-stu-id="3cac4-416">Increase time between retries of testing URL (#11015)</span></span>
- <span data-ttu-id="3cac4-417">Уточнено описание тестовых действий</span><span class="sxs-lookup"><span data-stu-id="3cac4-417">Update tests to accurately describe test actions.</span></span> <span data-ttu-id="3cac4-418">(№ 10928) (выражаем благодарность @romero126)</span><span class="sxs-lookup"><span data-stu-id="3cac4-418">(#10928) (Thanks @romero126!)</span></span>
- <span data-ttu-id="3cac4-419">Ненадежный тест TestAppDomainProcessExitEvenHandlerNotLeaking временно пропускается (№ 10827)</span><span class="sxs-lookup"><span data-stu-id="3cac4-419">Temporary skip the flaky test TestAppDomainProcessExitEvenHandlerNotLeaking (#10827)</span></span>
- <span data-ttu-id="3cac4-420">Тест на утечку в обработчике событий теперь выполняется стабильно (№ 10790)</span><span class="sxs-lookup"><span data-stu-id="3cac4-420">Make the event handler leaking test stable (#10790)</span></span>
- <span data-ttu-id="3cac4-421">Синхронизация прописных букв в CI YAML (№ 10767) (выражаем благодарность @RDIL)</span><span class="sxs-lookup"><span data-stu-id="3cac4-421">Sync capitalization in CI YAML (#10767) (Thanks @RDIL!)</span></span>
- <span data-ttu-id="3cac4-422">Добавлен тест для исправления, связанного с утечкой в обработчике событий (№ 10768)</span><span class="sxs-lookup"><span data-stu-id="3cac4-422">Add test for the event handler leaking fix (#10768)</span></span>
- <span data-ttu-id="3cac4-423">Добавлен тест Get-ChildItem (№ 10507) (выражаем благодарность @iSazonov)</span><span class="sxs-lookup"><span data-stu-id="3cac4-423">Add Get-ChildItem test (#10507) (Thanks @iSazonov!)</span></span>
- <span data-ttu-id="3cac4-424">Для устранения неоднозначности в тестах термин "флаг" заменен на "параметр" (№ 10666) (выражаем благодарность @romero126)</span><span class="sxs-lookup"><span data-stu-id="3cac4-424">Replace ambiguous language for tests from switch to parameter for accuracy (#10666) (Thanks @romero126!)</span></span>
- <span data-ttu-id="3cac4-425">Добавлена экспериментальная проверка в тесты ForEach-Object -Parallel (№ 10354) (выражаем благодарность @KirkMunro)</span><span class="sxs-lookup"><span data-stu-id="3cac4-425">Add experimental check to ForEach-Object -Parallel tests (#10354) (Thanks @KirkMunro!)</span></span>
- <span data-ttu-id="3cac4-426">Добавлены тесты для проверки Alpine (№ 10428)</span><span class="sxs-lookup"><span data-stu-id="3cac4-426">Update tests for Alpine validation (#10428)</span></span>

### <a name="build-and-package-improvements"></a><span data-ttu-id="3cac4-427">Улучшения сборки и упаковки</span><span class="sxs-lookup"><span data-stu-id="3cac4-427">Build and Package Improvements</span></span>

- <span data-ttu-id="3cac4-428">Исправлена проблема, связанная с подписанием пакета Nuget при сборке координированного пакета (№ 11316)</span><span class="sxs-lookup"><span data-stu-id="3cac4-428">Fix Nuget package signing for Coordinated Package build (#11316)</span></span>
- <span data-ttu-id="3cac4-429">Обновлены зависимости из коллекции PowerShell и NuGet (№ 11323)</span><span class="sxs-lookup"><span data-stu-id="3cac4-429">Update dependencies from PowerShell Gallery and NuGet (#11323)</span></span>
- <span data-ttu-id="3cac4-430">Обновление Microsoft.ApplicationInsights с версии 2.11.0 до версии 2.12.0 (№ 11305)</span><span class="sxs-lookup"><span data-stu-id="3cac4-430">Bump Microsoft.ApplicationInsights from 2.11.0 to 2.12.0 (#11305)</span></span>
- <span data-ttu-id="3cac4-431">Обновление Microsoft.CodeAnalysis.CSharp с версии 3.3.1 до версии 3.4.0 (№ 11265)</span><span class="sxs-lookup"><span data-stu-id="3cac4-431">Bump Microsoft.CodeAnalysis.CSharp from 3.3.1 to 3.4.0 (#11265)</span></span>
- <span data-ttu-id="3cac4-432">Обновлены пакеты для Debian 10 и 11 (№ 11236)</span><span class="sxs-lookup"><span data-stu-id="3cac4-432">Updates packages for Debian 10 and 11 (#11236)</span></span>
- <span data-ttu-id="3cac4-433">Включение экспериментальных возможностей только до выпуска RC (№ 11162)</span><span class="sxs-lookup"><span data-stu-id="3cac4-433">Only enable experimental features prior to RC (#11162)</span></span>
- <span data-ttu-id="3cac4-434">Изменена минимальная версия macOS (№ 11163)</span><span class="sxs-lookup"><span data-stu-id="3cac4-434">Update macOS minimum version (#11163)</span></span>
- <span data-ttu-id="3cac4-435">Обновление NJsonSchema с версии 10.0.27 до версии 10.0.28 (№ 11170)</span><span class="sxs-lookup"><span data-stu-id="3cac4-435">Bump NJsonSchema from 10.0.27 to 10.0.28 (#11170)</span></span>
- <span data-ttu-id="3cac4-436">Обновлены ссылки в файлах README.md и metadata.json для Preview.5 (№ 10854)</span><span class="sxs-lookup"><span data-stu-id="3cac4-436">Updating links in README.md and metadata.json for Preview.5 (#10854)</span></span>
- <span data-ttu-id="3cac4-437">Выбор файлов для тестов соответствия, относящихся к PowerShell (№ 10837)</span><span class="sxs-lookup"><span data-stu-id="3cac4-437">Select the files for compliance tests which are owned by PowerShell (#10837)</span></span>
- <span data-ttu-id="3cac4-438">Реализована возможность сборки пакета MSIX win7x86</span><span class="sxs-lookup"><span data-stu-id="3cac4-438">Allow win7x86 msix package to build.</span></span> <span data-ttu-id="3cac4-439">(внутреннее исправление № 10515)</span><span class="sxs-lookup"><span data-stu-id="3cac4-439">(Internal 10515)</span></span>
- <span data-ttu-id="3cac4-440">Разрешена передача семантических версий в функцию NormalizeVersion (№ 11087)</span><span class="sxs-lookup"><span data-stu-id="3cac4-440">Allow semantic versions to be passed to NormalizeVersion function (#11087)</span></span>
- <span data-ttu-id="3cac4-441">Обновление платформы .NET Core до версии 3.1-preview.3 (№ 11079)</span><span class="sxs-lookup"><span data-stu-id="3cac4-441">Bump .NET core framework to 3.1-preview.3 (#11079)</span></span>
- <span data-ttu-id="3cac4-442">Обновление PSReadLine с версии 2.0.0-beta5 до версии 2.0.0-beta6 в /src/Modules (№ 11078)</span><span class="sxs-lookup"><span data-stu-id="3cac4-442">Bump PSReadLine from 2.0.0-beta5 to 2.0.0-beta6 in /src/Modules (#11078)</span></span>
- <span data-ttu-id="3cac4-443">Обновление Newtonsoft.Json с версии 12.0.2 до версии 12.0.3 (№ 11037) (№ 11038)</span><span class="sxs-lookup"><span data-stu-id="3cac4-443">Bump Newtonsoft.Json from 12.0.2 to 12.0.3 (#11037) (#11038)</span></span>
- <span data-ttu-id="3cac4-444">Добавлены пакеты Debian 10, 11 и CentOS 8 (№ 11028)</span><span class="sxs-lookup"><span data-stu-id="3cac4-444">Add Debian 10, 11 and CentOS 8 packages (#11028)</span></span>
- <span data-ttu-id="3cac4-445">Отправка JSON-файла Build-Info с полем ReleaseDate (№ 10986)</span><span class="sxs-lookup"><span data-stu-id="3cac4-445">Upload Build-Info Json file with the ReleaseDate field (#10986)</span></span>
- <span data-ttu-id="3cac4-446">Обновление платформы .NET Core до версии 3.1-preview.2 (№ 10993)</span><span class="sxs-lookup"><span data-stu-id="3cac4-446">Bump .NET core framework to 3.1-preview.2 (#10993)</span></span>
- <span data-ttu-id="3cac4-447">Реализована возможность сборки пакета MSIX x86 (№ 10934)</span><span class="sxs-lookup"><span data-stu-id="3cac4-447">Enable build of x86 MSIX package (#10934)</span></span>
- <span data-ttu-id="3cac4-448">Обновлен URL-адрес скрипта установки пакета SDK dotnet в файле build.psm1 (№ 10927)</span><span class="sxs-lookup"><span data-stu-id="3cac4-448">Update the dotnet SDK install script URL in build.psm1 (#10927)</span></span>
- <span data-ttu-id="3cac4-449">Обновление Markdig.Signed с версии 0.17.1 до версии 0.18.0 (№ 10887)</span><span class="sxs-lookup"><span data-stu-id="3cac4-449">Bump Markdig.Signed from 0.17.1 to 0.18.0 (#10887)</span></span>
- <span data-ttu-id="3cac4-450">Обновление ThreadJob с версии 2.0.1 до версии 2.0.2 (№ 10886)</span><span class="sxs-lookup"><span data-stu-id="3cac4-450">Bump ThreadJob from 2.0.1 to 2.0.2 (#10886)</span></span>
- <span data-ttu-id="3cac4-451">Обновление манифеста AppX и модуля упаковки в соответствии с требованиями Microsoft Store (№ 10878)</span><span class="sxs-lookup"><span data-stu-id="3cac4-451">Update AppX Manifest and Packaging module to conform to MS Store requirements (#10878)</span></span>
- <span data-ttu-id="3cac4-452">Обновление ссылки на пакет для SDK PowerShell до версии preview.5 (внутреннее исправление № 10295)</span><span class="sxs-lookup"><span data-stu-id="3cac4-452">Update package reference for PowerShell SDK to preview.5 (Internal 10295)</span></span>
- <span data-ttu-id="3cac4-453">Обновление файла ThirdPartyNotices.txt (№ 10834)</span><span class="sxs-lookup"><span data-stu-id="3cac4-453">Update ThirdPartyNotices.txt (#10834)</span></span>
- <span data-ttu-id="3cac4-454">Обновление Microsoft.PowerShell.Native до версии 7.0.0-preview.3 (№ 10826)</span><span class="sxs-lookup"><span data-stu-id="3cac4-454">Bump Microsoft.PowerShell.Native to 7.0.0-preview.3 (#10826)</span></span>
- <span data-ttu-id="3cac4-455">Обновление Microsoft.ApplicationInsights с версии 2.10.0 до версии 2.11.0 (№ 10608)</span><span class="sxs-lookup"><span data-stu-id="3cac4-455">Bump Microsoft.ApplicationInsights from 2.10.0 to 2.11.0 (#10608)</span></span>
- <span data-ttu-id="3cac4-456">Обновление NJsonSchema с версии 10.0.24 до версии 10.0.27 (№10756)</span><span class="sxs-lookup"><span data-stu-id="3cac4-456">Bump NJsonSchema from 10.0.24 to 10.0.27 (#10756)</span></span>
- <span data-ttu-id="3cac4-457">Добавление поддержки MacPorts в систему сборки (№ 10736) (выражаем благодарность @Lucius-Q-User)</span><span class="sxs-lookup"><span data-stu-id="3cac4-457">Add MacPorts support to the build system (#10736) (Thanks @Lucius-Q-User!)</span></span>
- <span data-ttu-id="3cac4-458">Обновление PackageManagement с версии 1.4.4 до версии 1.4.5 (№ 10728)</span><span class="sxs-lookup"><span data-stu-id="3cac4-458">Bump PackageManagement from 1.4.4 to 1.4.5 (#10728)</span></span>
- <span data-ttu-id="3cac4-459">Обновление NJsonSchema с версии 10.0.23 до версии 10.0.24 (№ 10635)</span><span class="sxs-lookup"><span data-stu-id="3cac4-459">Bump NJsonSchema from 10.0.23 to 10.0.24 (#10635)</span></span>
- <span data-ttu-id="3cac4-460">Добавление переменной среды для различения телеметрии клиента и сервера в MSI (№ 10612)</span><span class="sxs-lookup"><span data-stu-id="3cac4-460">Add environment variable to differentiate client/server telemetry in MSI (#10612)</span></span>
- <span data-ttu-id="3cac4-461">Обновление PSDesiredStateConfiguration с версии 2.0.3 до версии 2.0.4 (№ 10603)</span><span class="sxs-lookup"><span data-stu-id="3cac4-461">Bump PSDesiredStateConfiguration from 2.0.3 to 2.0.4 (#10603)</span></span>
- <span data-ttu-id="3cac4-462">Обновление Microsoft.CodeAnalysis.CSharp с версии 3.2.1 до версии 3.3.1 (№ 10607)</span><span class="sxs-lookup"><span data-stu-id="3cac4-462">Bump Microsoft.CodeAnalysis.CSharp from 3.2.1 to 3.3.1 (#10607)</span></span>
- <span data-ttu-id="3cac4-463">Обновление до версии .Net Core 3.0 RTM (№ 10604) (выражаем благодарность @bergmeister)</span><span class="sxs-lookup"><span data-stu-id="3cac4-463">Update to .Net Core 3.0 RTM (#10604) (Thanks @bergmeister!)</span></span>
- <span data-ttu-id="3cac4-464">Обновление версии средства упаковки MSIX в соответствии с требованиями Магазина Windows (№ 10588)</span><span class="sxs-lookup"><span data-stu-id="3cac4-464">Update MSIX packaging so the version to Windows Store requirements (#10588)</span></span>
- <span data-ttu-id="3cac4-465">Обновление PowerShellGet с версии 2.2 до версии 2.2.1 (№ 10382)</span><span class="sxs-lookup"><span data-stu-id="3cac4-465">Bump PowerShellGet version from 2.2 to 2.2.1 (#10382)</span></span>
- <span data-ttu-id="3cac4-466">Обновление PackageManagement с версии 1.4.3 до версии 1.4.4 (№ 10383)</span><span class="sxs-lookup"><span data-stu-id="3cac4-466">Bump PackageManagement version from 1.4.3 to 1.4.4 (#10383)</span></span>
- <span data-ttu-id="3cac4-467">Обновление файлов README.md и metadata.json для версии 7.0.0-preview.4 (внутреннее исправление № 10011)</span><span class="sxs-lookup"><span data-stu-id="3cac4-467">Update README.md and metadata.json for 7.0.0-preview.4 (Internal 10011)</span></span>
- <span data-ttu-id="3cac4-468">Обновление версии .Net Core 3.0 с предварительной версии 9 до RC1 (№ 10552) (выражаем благодарность @bergmeister)</span><span class="sxs-lookup"><span data-stu-id="3cac4-468">Upgrade .Net Core 3.0 version from Preview 9 to RC1 (#10552) (Thanks @bergmeister!)</span></span>
- <span data-ttu-id="3cac4-469">Исправлена проблема, связанная с формированием списка экспериментальных возможностей (внутреннее исправление № 9996)</span><span class="sxs-lookup"><span data-stu-id="3cac4-469">Fix ExperimentalFeature list generation (Internal 9996)</span></span>
- <span data-ttu-id="3cac4-470">Обновление PSReadLine с версии 2.0.0-beta4 до версии 2.0.0-beta5 (№ 10536)</span><span class="sxs-lookup"><span data-stu-id="3cac4-470">Bump PSReadLine version from 2.0.0-beta4 to 2.0.0-beta5 (#10536)</span></span>
- <span data-ttu-id="3cac4-471">Исправление скрипта сборки выпуска с целью задания тега выпуска</span><span class="sxs-lookup"><span data-stu-id="3cac4-471">Fix release build script to set release tag</span></span>
- <span data-ttu-id="3cac4-472">Обновление Microsoft.PowerShell.Native до версии 7.0.0-preview.2 (№ 10519)</span><span class="sxs-lookup"><span data-stu-id="3cac4-472">Update version of Microsoft.PowerShell.Native to 7.0.0-preview.2 (#10519)</span></span>
- <span data-ttu-id="3cac4-473">Обновление до версии Netcoreapp3.0 preview9 (№ 10484) (выражаем благодарность @bergmeister)</span><span class="sxs-lookup"><span data-stu-id="3cac4-473">Upgrade to Netcoreapp3.0 preview9 (#10484) (Thanks @bergmeister!)</span></span>
- <span data-ttu-id="3cac4-474">Ежедневная координированная сборка определяется как ежедневная (№ 10464)</span><span class="sxs-lookup"><span data-stu-id="3cac4-474">Make sure the daily coordinated build, knows it is a daily build (#10464)</span></span>
- <span data-ttu-id="3cac4-475">Обновление объединенной сборки пакета для выпуска ежедневных сборок (№ 10449)</span><span class="sxs-lookup"><span data-stu-id="3cac4-475">Update the combined package build to release the daily builds (#10449)</span></span>
- <span data-ttu-id="3cac4-476">Удалена ссылка на appveyor (№ 10445) (выражаем благодарность @RDIL)</span><span class="sxs-lookup"><span data-stu-id="3cac4-476">Remove appveyor reference (#10445) (Thanks @RDIL!)</span></span>
- <span data-ttu-id="3cac4-477">Обновление NJsonSchema с версии 10.0.22 до версии 10.0.23 (№ 10421)</span><span class="sxs-lookup"><span data-stu-id="3cac4-477">Bump NJsonSchema version from 10.0.22 to 10.0.23 (#10421)</span></span>
- <span data-ttu-id="3cac4-478">Отменено удаление папки сборки linux-x64, так как она требуется для некоторых зависимостей для Alpine (№ 10407)</span><span class="sxs-lookup"><span data-stu-id="3cac4-478">Remove the deletion of linux-x64 build folder because some dependencies for Alpine need it (#10407)</span></span>

### <a name="documentation-and-help-content"></a><span data-ttu-id="3cac4-479">Документация и содержимое справки</span><span class="sxs-lookup"><span data-stu-id="3cac4-479">Documentation and Help Content</span></span>

- <span data-ttu-id="3cac4-480">Рефакторинг журналов изменений в один журнал на каждый выпуск (№ 11165)</span><span class="sxs-lookup"><span data-stu-id="3cac4-480">Refactor change logs into one log per release (#11165)</span></span>
- <span data-ttu-id="3cac4-481">Исправлены ссылки перенаправления для справочной документации PowerShell 7 в Интернете (№ 11071)</span><span class="sxs-lookup"><span data-stu-id="3cac4-481">Fix FWLinks for PowerShell 7 online help documents (#11071)</span></span>
- <span data-ttu-id="3cac4-482">Обновлен файл CONTRIBUTING.md (№ 11096) (выражаем благодарность @mklement0)</span><span class="sxs-lookup"><span data-stu-id="3cac4-482">Update CONTRIBUTING.md (#11096) (Thanks @mklement0!)</span></span>
- <span data-ttu-id="3cac4-483">Исправлены ссылки на документацию по установке в README.md (№ 11083)</span><span class="sxs-lookup"><span data-stu-id="3cac4-483">Fix installation doc links in README.md (#11083)</span></span>
- <span data-ttu-id="3cac4-484">Добавлены примеры в скрипт install-powershell.ps1 (№ 11024) (выражаем благодарность @kilasuit)</span><span class="sxs-lookup"><span data-stu-id="3cac4-484">Adds examples to install-powershell.ps1 script (#11024) (Thanks @kilasuit!)</span></span>
- <span data-ttu-id="3cac4-485">Исправления, касающиеся Select-String emphasis и Import-DscResource, в файле CHANGELOG.md (№ 10890)</span><span class="sxs-lookup"><span data-stu-id="3cac4-485">Fix to Select-String emphasis and Import-DscResource in CHANGELOG.md (#10890)</span></span>
- <span data-ttu-id="3cac4-486">Удалена устаревшая ссылка из файла powershell-beginners-guide.md (№ 10926)</span><span class="sxs-lookup"><span data-stu-id="3cac4-486">Remove the stale link from powershell-beginners-guide.md (#10926)</span></span>
- <span data-ttu-id="3cac4-487">Объединены стабильные и служебные журналы изменений (№ 10527)</span><span class="sxs-lookup"><span data-stu-id="3cac4-487">Merge stable and servicing change logs (#10527)</span></span>
- <span data-ttu-id="3cac4-488">Обновление, касающееся используемой версии .NET, в документации по сборке (№ 10775) (выражаем благодарность @Greg-Smulko)</span><span class="sxs-lookup"><span data-stu-id="3cac4-488">Update used .NET version in build docs (#10775) (Thanks @Greg-Smulko!)</span></span>
- <span data-ttu-id="3cac4-489">Ссылки на MSDN заменены на ссылки на docs.microsoft.com в powershell-beginners-guide.md (№ 10778) (выражаем благодарность @iSazonov)</span><span class="sxs-lookup"><span data-stu-id="3cac4-489">Replace links from MSDN to docs.microsoft.com in powershell-beginners-guide.md (#10778) (Thanks @iSazonov!)</span></span>
- <span data-ttu-id="3cac4-490">Исправлена неработающая ссылка на обзор DSC (№ 10702)</span><span class="sxs-lookup"><span data-stu-id="3cac4-490">Fix broken DSC overview link (#10702)</span></span>
- <span data-ttu-id="3cac4-491">В Support_Question.md добавлена ссылка на Stack Overflow как на еще один ресурс сообщества (№ 10638) (выражаем благодарность @mklement0)</span><span class="sxs-lookup"><span data-stu-id="3cac4-491">Update Support_Question.md to link to Stack Overflow as another community resource (#10638) (Thanks @mklement0!)</span></span>
- <span data-ttu-id="3cac4-492">В шаблон запроса распространения добавлена архитектура процессора (№ 10661)</span><span class="sxs-lookup"><span data-stu-id="3cac4-492">Add processor architecture to distribution request template (#10661)</span></span>
- <span data-ttu-id="3cac4-493">В обучающую документацию PowerShell добавлена новая книга о PowerShell MoL (№ 10602)</span><span class="sxs-lookup"><span data-stu-id="3cac4-493">Add new PowerShell MoL book to learning PowerShell docs (#10602)</span></span>
- <span data-ttu-id="3cac4-494">Обновлен файл README.md и метаданные для выпусков v6.1.6 и v6.2.3 (№ 10523)</span><span class="sxs-lookup"><span data-stu-id="3cac4-494">Update README.md and metadata for v6.1.6 and v6.2.3 releases (#10523)</span></span>
- <span data-ttu-id="3cac4-495">Исправлена опечатка в файле README.md (№ 10465) (выражаем благодарность @vedhasp)</span><span class="sxs-lookup"><span data-stu-id="3cac4-495">Fix a typo in README.md (#10465) (Thanks @vedhasp!)</span></span>
- <span data-ttu-id="3cac4-496">В обучающие ресурсы добавлена ссылка на модуль PSKoans (№ 10369) (выражаем благодарность @vexx32)</span><span class="sxs-lookup"><span data-stu-id="3cac4-496">Add a reference to PSKoans module to Learning Resources documentation (#10369) (Thanks @vexx32!)</span></span>
- <span data-ttu-id="3cac4-497">Обновление файлов README.md и metadata.json для версии 7.0.0-preview.3 (№ 10393)</span><span class="sxs-lookup"><span data-stu-id="3cac4-497">Update README.md and metadata.json for 7.0.0-preview.3 (#10393)</span></span>
