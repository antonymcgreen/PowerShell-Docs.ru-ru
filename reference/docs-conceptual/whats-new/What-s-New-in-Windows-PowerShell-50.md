...
- Новые командлеты в модуле [PSDesiredStateConfiguration](https://technet.microsoft.com/library/dn391651(v=wps.640).aspx):
  - [Update-DscConfiguration](https://technet.microsoft.com/library/mt143541(v=wps.630).aspx)  Выполняет проверку сервера по запросу на включение внесенных изменений.
  - [Stop-DscConfiguration](https://technet.microsoft.com/library/mt143542(v=wps.630).aspx)  Останавливает конфигурацию, которая уже выполняется.
  - [Remove-DscConfigurationDocument](https://technet.microsoft.com/library/mt143544(v=wps.630).aspx)  Позволяет удалять документы конфигурации на разных этапах (Ожидание предыдущего и текущего).
...
- Усовершенствования гибкости:
  ...
  - Поддержка управления версиями:  DSC теперь может распознать документ, который был создан в более новой версии клиента (в состав [WMF 5.0](https://aka.ms/wmf5download)).
...
- WMF 5.0 (PowerShell 5.0) включает в себя **Pester** модуля.  Pester — это платформа модульного тестирования для PowerShell. Он предоставляет несколько ключевых слов простой в использовании, позволяющие создавать тесты для сценариев. 
...
- Теперь можно редактировать удаленные сценарии Windows PowerShell и файлы в локальной копии интегрированной среды Сценариев Windows PowerShell, выполнив Enter-PSSession для открытия удаленного сеанса на компьютере, где хранятся файлы, которые нужно изменить, а затем выполнив **PSEdit \<путь и имя файла на удаленном компьютере\>**. Эта функция упрощает редактирование файлов Windows PowerShell, хранящихся в Windows Server только с основными серверными компонентами, где запуск интегрированной среды сценариев Windows PowerShell невозможен.
...
- [Новые API-интерфейсы командлетов и размещения](#new-cmdlet-and-hosting-apis)
...
Рабочие процессы Windows PowerShell привносят возможности Windows Workflow Foundation в Windows PowerShell. Рабочие процессы можно записывать на языке XAML или Windows PowerShell и запускать их так же, как командлеты. [Get-Command](https://technet.microsoft.com/library/59c6d302-6e8c-48b7-a6f6-f0172df936ad) командлет получает команды рабочего процесса и [Get-Help](https://technet.microsoft.com/library/1f46eeb4-49d7-4bec-bb29-395d9b42f54a) получает справку для рабочих процессов.
...
Дополнительные сведения о поддержке справки в Интернете, см. в разделе [поддержка справки в Интернете](/powershell/developer/module/supporting-online-help) в документации по Microsoft.
