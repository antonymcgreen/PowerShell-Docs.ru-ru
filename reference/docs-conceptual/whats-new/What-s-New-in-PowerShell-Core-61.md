...
`Import-Csv` — выполнение этого командлета также было значительно ускорено после регрессии из Windows PowerShell.
...
`Update-Help` теперь по умолчанию сохраняет справку в папку пользователя.
