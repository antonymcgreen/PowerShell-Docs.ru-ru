...
Дополнительные сведения о .NET Standard см. в [Блог .NET][], в этом видео на [YouTube][] и [Вопросы и ответы][] на GitHub.
...
- `$PSVersionTable` имеет четыре новых свойства:
  ...
  - `Platform`: возвращается `[System.Environment]::OSVersion.Platform`, имеет значение `Win32NT` в Windows, `Unix` на macOS и `Unix` в Linux.
- Удалено свойство `BuildVersion` из `$PSVersionTable`.
  Это свойство было жестко привязано к версии сборки Windows.
  Вместо этого мы рекомендуем использовать `GitCommitId` для получения точной версии сборки PowerShell Core. (#3877) (выражаем благодарность [@iSazonov](https://github.com/iSazonov))
...
- Добавлен `GitCommitId` в баннер PowerShell Core.
  Теперь вам не нужно выполнять `$PSVersionTable` сразу после запуска PowerShell, чтобы получить версию. (#3916) (выражаем благодарность [@iSazonov](https://github.com/iSazonov))
...
- Добавлены параметры проверки подлинности для веб-командлетов. (#5052) (выражаем благодарность [@markekraus](https://github.com/markekraus))
  - Добавление `-Authentication` , предоставляющий три параметра: Basic, OAuth и Bearer.
...
- Добавлен`-ResponseHeadersVariable` в `Invoke-RestMethod`, чтобы включить захват заголовков ответа. (#4888) (выражаем благодарность [@markekraus](https://github.com/markekraus))
...
- Исправлены веб-командлеты `-SkipHeaderValidation`, чтобы работать с нестандартными заголовками агента пользователя. (#4479 и #4512) (выражаем благодарность [@markekraus](https://github.com/markekraus))
...
- Удален `AliasProperty "Count"`, определенный для `System.Array`.
  Это удаляет лишнее свойство `Count` для некоторых выходных данных `ConvertFrom-Json`. (#3231) (выражаем благодарность [@PetSerAl](https://github.com/PetSerAl))
...
- Включено автоматическое завершение аргументов для `-ExcludeProperty` и `-ExpandProperty` объекта `Select-Object`. (#3443) (выражаем благодарность [@iSazonov](https://github.com/iSazonov))
...
Полный список исправлений и изменений см. в нашем [журнал изменений][] на GitHub.
...
Если вы не хотите использовать эту телеметрию, просто создайте переменную среды `POWERSHELL_TELEMETRY_OPTOUT` с одним из следующих значений: `true`, `1` или `yes`.
В этом случае обход всей телеметрии выполняется даже до первого запуска PowerShell.
Мы также планируем публиковать данные телеметрии и основанные на них аналитические сведения на [панели мониторинга сообщества][community-dashboard].
