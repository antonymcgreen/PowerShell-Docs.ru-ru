...
> [!NOTE]
> Этот документ относится к поддержке PowerShell Core 6 и PowerShell 7. Windows PowerShell (1.0–5.1) является компонентом операционной системы Windows. Компоненты получают ту же поддержку, что и их родительский продукт или платформа. Дополнительные сведения см. в разделе [Сведения о жизненном цикле продукта и служб](/lifecycle/products/).
