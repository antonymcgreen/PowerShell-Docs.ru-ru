...
В этом разделе описывается добавление псевдонимов, расширение подстановочных знаков и сообщения справки в параметры командлета Stop-Proc (описывается в разделе [Создание командлета, изменяющего систему](./creating-a-cmdlet-that-modifies-the-system.md)).

Этот командлет Stop-Proc пытается прерывать процессы, полученные с помощью командлета Get-Proc (описывается в разделе [Создание первого командлета](./creating-a-cmdlet-without-parameters.md)).
...
Следующий код из этого командлета Stop-Proc определяет `HelpMessage` ключевое слово атрибута для `Name` параметра.
...
- Запустите Windows PowerShell и используйте Stop-Proc, чтобы прерывать процесс с помощью псевдонима ProcessName для `Name` параметра.
