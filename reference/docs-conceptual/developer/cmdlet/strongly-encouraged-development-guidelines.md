...
Если параметр принимает только значение `true` и `false` , определите параметр как тип [System. Management. Automation. переключатель](/dotnet/api/System.Management.Automation.SwitchParameter). Параметр Switch обрабатывается так, как `true` если бы он был указан в команде. Если параметр не включен в команду, Windows PowerShell считает значение параметра равным `false` . Не определяйте логические параметры.
...
Если командлет позволяет пользователю указать файл или источник данных, он должен определить параметр типа [System. String](/dotnet/api/System.String). Если поддерживается более одного диска, тип должен быть массивом. Имя параметра должно иметь `Path` псевдоним `PSPath` . Кроме того, `Path` параметр должен поддерживать символы-шаблоны. Если поддержка символов-шаблонов не требуется, определите `LiteralPath` параметр.
