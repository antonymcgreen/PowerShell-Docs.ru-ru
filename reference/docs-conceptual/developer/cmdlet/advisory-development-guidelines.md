...
Класс для командлета обычно определяется в .NET Framework пространстве имен, которое добавляет ". Commands в пространство имен, представляющее продукт, в котором выполняется командлет. Например, командлеты, которые входят в состав Windows PowerShell, определяются в пространстве имен `Microsoft.PowerShell.Commands`.
