...
Ресурсы, предоставляемые веб-службой OData управления, определяются путем создания файла MOF, который использует схему общедоступных ресурсов. Каждый ресурс определяется как класс в файле, а свойства определяются как члены класса. Дополнительные сведения о схеме, используемой в MOF-файле, см. в разделе [схема общедоступного ресурса](./public-resource-schema.md).

## <a name="example-mof-file"></a>Пример MOF-файла
