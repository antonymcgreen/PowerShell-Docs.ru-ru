...
В этом разделе показано, как настроить политику авторизации на основе ролей для примера реализации интерфейса [Microsoft. Management. OData. кустомаусоризатион](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) , описанного в статье [Реализация настраиваемой авторизации для управления. Расширение IIS OData](./implementing-custom-authorization-for-a-management-odata-web-service.md).
...
Этот файл определяет политику авторизации для приложения. Роли определяются с помощью узлов `Group`. Узел `Group` определяет команды Windows PowerShell, которые могут выполняться пользователям, назначенным этой группе. Пользователи назначаются группам с помощью `User` узлов.

В этих примерах вы добавите модуль к администратору `Group` и добавите пользователя в каждую группу.
...
2. Файл содержит два узла `Group`. Они представляют две роли, используемые в этом примере: `NonAdminGroup` и `AdminGroup`.

   Сразу после закрывающего тега `Cmdlets` в первом узле `Group` добавьте следующий код XML:
