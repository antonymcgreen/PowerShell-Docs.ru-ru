...
Использование веб-службы Windows PowerShell требует от стороннего разработчика реализовать интерфейс [Microsoft. Management. OData. кустомаусоризатион](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) для предоставления командлетов Windows PowerShell. Этот интерфейс выполняет авторизацию пользователя для веб-службы. После написания кода для реализации интерфейса необходимо скомпилировать его в библиотеку DLL, которая будет использоваться в веб-приложении.
...
В следующем примере создается класс, который анализирует политику авторизации в XML-файле.
...
```
 ...
 Следующие классы представляют группы и пользователей соответственно.
