...
Обратите внимание на пустые строки между закрывающим **#>** и оператором `Param`. В скрипте, не имеющем оператора `Param`, между последним комментарием в разделе справки и первым объявлением функции должно быть по крайней мере две пустые строки. Без этих пустых строк Get-Help связывает раздел справки с функцией, а не со сценарием.
...
Следующая команда получает справку по скрипту. Так как скрипт не имеет n каталога, указанного в переменной среды PATH, команда Get-Help, которая получает справку по скрипту, должна указать путь к скрипту.
