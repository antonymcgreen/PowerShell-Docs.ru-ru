...
Вам нужно установить клиент (`ssh.exe`) и сервер (`sshd.exe`), чтобы можно было поэкспериментировать с удаленным взаимодействием к компьютерам и с них.
...
- Команда sudo не работает во входящем удаленном сеансе на компьютер Linux.
