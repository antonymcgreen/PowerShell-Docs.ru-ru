...
Развертывание опрашивающего сервера можно упростить, подготовив службу с использованием скрипта настройки DSC. В этот документ включены скрипты настройки, которые можно использовать для развертывания узла сервера, готового к внедрению в рабочую среду. Для использования скриптов настройки требуется модуль DSC, не входящий в состав Windows Server. Название модуля — **xPSDesiredStateConfiguration**; модуль содержит ресурс DSC **xDscWebService**. Модуль xPSDesiredStateConfiguration можно скачать [здесь](https://github.com/dsccommunity/xPSDesiredStateConfiguration).
...
- **Windows Server 2012** — файл всегда будет иметь расширение `.mdb`.
- **Windows Server 2012 R2** — файл по умолчанию будет иметь расширение `.edb` до тех пор, пока в конфигурации не будет указано расширение `.mdb`.
