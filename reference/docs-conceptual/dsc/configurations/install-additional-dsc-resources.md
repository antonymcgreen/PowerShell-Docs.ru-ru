...
|Ресурс  |Описание  |
|---------|---------|
|**Файл**|Управляет состоянием файлов и каталогов. Копирует файлы из **Источника** в **Место назначения** и обновляет их при изменении **Источника** путем сравнения дат, контрольных сумм и хэшей.|
|**Архив**|Распаковывает архивы и указывает местоположение. Проверяет архивы с указанной **Контрольной суммой**.|
|**Среда**|Управляет переменными среды.|
|**Группирование**|Управляет локальными группами и контролирует членство в группах.|
|**Журнал**|Записывает сообщения в журнал событий `Microsoft-Windows-Desired State Configuration/Analytic`.|
|**Пакет**|Устанавливает или удаляет пакеты, используя **Аргументы**, **LogPath**, **ReturnCode**, другие параметры.|
|**Реестр**|Управляет разделами и значениями реестра.|
|**Скрипт**|Позволяет разрабатывать собственные блоки сценариев [get-test-set](../resources/get-test-set.md).|
|**Служба**|Настраивает службы Windows.|
|**Пользователь** |Управляет локальными пользователями и атрибутами.|
