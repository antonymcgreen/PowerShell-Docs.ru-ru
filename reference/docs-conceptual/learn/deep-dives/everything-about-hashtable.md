...
## <a name="keys-are-just-strings"></a>Ключи — это просто строки

Я раньше не хотел использовать этот тангенс, но ключи — это просто строки. Таким образом, мы можем заключить в кавычки все, что угодно, и сделать это ключом.
...
Технически ваш ключ необязательно должен быть строкой, однако его проще воспринимать, если используются только строки.
...
Далее я привожу список всех рассмотренных тем на случай, если вы захотите вернуться к любой из них еще раз. В принципе, об этом стоило сказать в самом начале, однако эта статья писалась постепенно и снабжалась примерами, которые основывались на более ранних данных.
