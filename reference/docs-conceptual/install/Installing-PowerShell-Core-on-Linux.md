...
Поддерживает [Ubuntu 14.04][u14], [Ubuntu 16.04][u16], [Ubuntu 18.04][u1804], [Ubuntu 18.10][u1810],  [Debian 9][deb9],
[CentOS 7][cos], [Red Hat Enterprise Linux (RHEL) 7][rhel7], [openSUSE 42.3][opensuse], [openSUSE Leap 15][opensuse],
[Fedora 27][fedora], [Fedora 28][fedora] и [Arch Linux][arch].
...
Скачайте пакет Debian `powershell_6.2.0-1.ubuntu.14.04_amd64.deb`
...
Скачайте пакет Debian `powershell_6.2.0-1.ubuntu.16.04_amd64.deb`
со страницы [выпусков][] на компьютер с Ubuntu.
...
Скачайте пакет Debian `powershell_6.2.0-1.ubuntu.18.04_amd64.deb`
со страницы [выпусков][] на компьютер с Ubuntu.
...
> [!NOTE]
> Так как 18.10 — [промежуточная версия](https://www.ubuntu.com/about/release-cycle), она только [поддерживается сообществом](https://docs.microsoft.com/en-us/powershell/scripting/powershell-support-lifecycle?view=powershell-6).
...
Скачайте пакет Debian `powershell_6.2.0-1.debian.9_amd64.deb`
...
С помощью [CentOS 7][] скачайте пакет RPM `powershell-6.2.0-1.rhel.7.x86_64.rpm`
...
Скачайте пакет RPM `powershell-6.2.0-1.rhel.7.x86_64.rpm`
...
Например, наш [Dockerfile для Amazon Linux][amazon-dockerfile] сначала устанавливает зависимости, а затем извлекает архив Linux `tar.gz`.
