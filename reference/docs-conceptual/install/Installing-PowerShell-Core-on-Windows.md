...
Чтобы установить PowerShell в Windows скачайте [актуальный][https://github.com/PowerShell/PowerShell/releases/tag/v7.1.2] пакет установки с GitHub. Также доступна актуальная [предварительная версия][]. Прокрутите страницу вниз до раздела **ресурсов**. Раздел **ресурсов** может быть свернут. В таком случае щелкните его, чтобы развернуть.
