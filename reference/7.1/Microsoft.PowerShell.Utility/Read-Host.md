...
### Пример 3. входные данные маски и в виде строки с открытым текстом

В этом примере отображается строка "введите пароль:" в качестве подсказки. При вводе значения в `*` консоли вместо входных данных появятся звездочки (). При нажатии клавиши Ввод значение сохраняется как объект **строкового** текста в `$pwd_string` переменной.
...
Указывает, что командлет отображает звездочки ( `*` ) вместо символов, которые пользователь вводит в качестве входных данных. При использовании этого параметра выходные данные `Read-Host` командлета являются **строковым** объектом.
Это позволяет безопасно запрашивать пароль, который возвращается в виде обычного текста, а не **SecureString** .
