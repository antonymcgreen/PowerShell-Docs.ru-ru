...
### Пример 10. Возврат значения даты, интерпретируемого как время в формате UTC

В этом примере показано, как интерпретировать значение даты как эквивалент UTC. Для этого примера этот компьютер настроен на **стандартное тихоокеанское время**. По умолчанию `Get-Date` возвращает значения для этого часового пояса. Используйте параметр **асутк** , чтобы преобразовать значение в время в формате UTC.
...
Дата и время, представленные в секундах с 1 января 1970 0:00:00.
...
| `%e` | День месяца — предшествует пробел, если только одна цифра           | \<space\>5.0               |
...
| `%S` | Секунды                                                                 | 05                       |
| `%s` | Секунды, прошедшие с 1 января 1970 00:00:00                          | 1150451174               |
