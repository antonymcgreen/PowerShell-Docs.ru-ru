...
Указывает преобразуемый путь, который требуется разрешить.
Значение параметра **LiteralPath** используется в точности как типизированное.
...
Указывает путь PowerShell для разрешения.
Этот параметр обязателен.
Можно также передать строку пути в `Resolve-Path` .
...
Строку, содержащую путь к этому командлету, можно передать по конвейеру.
