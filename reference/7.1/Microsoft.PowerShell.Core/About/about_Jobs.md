...
Начиная с PowerShell 6,0 можно использовать амерсанд ( `&` ) в конце конвейера для запуска фонового задания. Следующая команда функционально эквивалентна команде выше.

```powershell
$job = Get-Process &
```

Амперсанд ( `&` ) называется оператором фона. Дополнительные сведения см. в разделе [оператор Background](about_Operators.md#background-operator-).
