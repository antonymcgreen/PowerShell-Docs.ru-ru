...
Начиная с версии PowerShell 6,0 можно использовать оператор Background ( `&` ) в конце конвейера для запуска фонового задания. Дополнительные сведения см. в разделе [оператор Background](about_Operators.md#background-operator-).

Использование оператора Background функционально эквивалентно использованию `Start-Job` командлета в предыдущем примере.
