...
Команда вызывает метод `Invoke-WebRequest` с **ErrorAction** **остановки** , который заставляет `Invoke-WebRequest` вызывать завершающую ошибку при любых неудачных запросах. Завершающая ошибка перехвачена `catch` блоком, который извлекает **StatusCode** из объекта **исключения** .
...
> [!NOTE]
> На платформах, отличных от Windows, невозможно предоставить в `'Tls, Tls12'` качестве параметра.
...
Этот параметр обязателен. **URI** имени параметра является необязательным.
...
Дополнительные сведения о типе объекта **Microsoft. PowerShell. Commands. басичтмлвебреспонсеобжект** см. в разделе [басичтмлвебреспонсеобжект](/dotnet/api/microsoft.powershell.commands.basichtmlwebresponseobject).

Дополнительные сведения о том, как .NET предоставляет прокси-службы для PowerShell, см. в разделе [доступ к Интернету через прокси-сервер](/dotnet/framework/network-programming/accessing-the-internet-through-a-proxy).
