...
> [!IMPORTANT]
> Чтобы убедиться в том, что возвращаются полные сведения о форматировании типов, следует всегда включать параметр **PowerShellVersion** со значением `5.1` при использовании локального вызова `Get-FormatData` . Если параметр и значение опущены, вы не сможете получить все правильные сведения о типе.
