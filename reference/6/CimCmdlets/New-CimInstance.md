...
`Get-CimInstance`Затем командлет извлекает определенный единственный экземпляр. `Invoke-CimMethod`Командлет вызывает метод метода **GetOwner** with для полученного экземпляра.
