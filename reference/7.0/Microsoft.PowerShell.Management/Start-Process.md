...
Указывает расположение, в котором должен начинаться новый процесс. По умолчанию это расположение исполняемого файла или документа, который запускается. Указанный путь рассматривается как литеральный путь. Подстановочные знаки не поддерживаются. Путь необходимо заключить в одинарные кавычки ( `'` ), если имя пути содержит символы, которые будут интерпретироваться как подстановочные знаки.
