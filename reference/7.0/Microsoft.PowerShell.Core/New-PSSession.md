...
### <span data-ttu-id="09bae-109">URI</span><span class="sxs-lookup"><span data-stu-id="09bae-109">Uri</span></span>
...
### <span data-ttu-id="09bae-115">сшхоссашпарам</span><span class="sxs-lookup"><span data-stu-id="09bae-115">SSHHostHashParam</span></span>
...
### <span data-ttu-id="09bae-140">Пример 3. Создание сеансов на нескольких компьютерах</span><span class="sxs-lookup"><span data-stu-id="09bae-140">Example 3: Create sessions on multiple computers</span></span>
...
### <span data-ttu-id="09bae-152">Пример 5. Создание сеанса на основе существующего сеанса</span><span class="sxs-lookup"><span data-stu-id="09bae-152">Example 5: Create a session based on an existing session</span></span>
...
### <span data-ttu-id="09bae-183">Пример 11. Создание параметра сеанса</span><span class="sxs-lookup"><span data-stu-id="09bae-183">Example 11: Create a session option</span></span>
...
### <span data-ttu-id="09bae-276">-Credential</span><span class="sxs-lookup"><span data-stu-id="09bae-276">-Credential</span></span>
...
### <span data-ttu-id="09bae-297">-HostName</span><span class="sxs-lookup"><span data-stu-id="09bae-297">-HostName</span></span>
...
### <span data-ttu-id="09bae-306">-Name</span><span class="sxs-lookup"><span data-stu-id="09bae-306">-Name</span></span>
...
### <span data-ttu-id="09bae-381">-KeyFilePath</span><span class="sxs-lookup"><span data-stu-id="09bae-381">-KeyFilePath</span></span>
...
### <span data-ttu-id="09bae-391">-UserName</span><span class="sxs-lookup"><span data-stu-id="09bae-391">-UserName</span></span>
...
<span data-ttu-id="09bae-402">Этот параметр принимает массив хэш-таблиц, где каждая таблица Hashtable содержит один или несколько параметров соединения, необходимых для установления подключения Secure Shell (имя узла, порт, имя пользователя, KeyFilePath, подсистема).</span><span class="sxs-lookup"><span data-stu-id="09bae-402">This parameter takes an array of hashtables where each hashtable contains one or more connection parameters needed to establish a Secure Shell (SSH) connection (HostName, Port, UserName, KeyFilePath, Subsystem).</span></span>

<span data-ttu-id="09bae-403">Параметры соединения Hashtable аналогичны параметрам, заданным для набора параметров **HostName** .</span><span class="sxs-lookup"><span data-stu-id="09bae-403">The hashtable connection parameters are the same as defined for the **HostName** parameter set.</span></span>
<span data-ttu-id="09bae-404">Обратите внимание, что порядок ключей в хэш-таблице приводит к тому, что имя пользователя и порт используются для соединения, в котором используется последнее указанное значение.</span><span class="sxs-lookup"><span data-stu-id="09bae-404">Note that the order of the keys in the hashtable result in user name and port being used for the connection where the last one specified is used.</span></span>  <span data-ttu-id="09bae-405">Например, если используется `@{UserName="first";HostName="second@host"}` , `second` будет использоваться имя пользователя.</span><span class="sxs-lookup"><span data-stu-id="09bae-405">For example, if you use `@{UserName="first";HostName="second@host"}`, then the user name `second` will be used.</span></span>  <span data-ttu-id="09bae-406">Однако если используется `@{HostName="second@host:22";Port=23}` , будет использоваться порт 23.</span><span class="sxs-lookup"><span data-stu-id="09bae-406">However, if you use `@{HostName="second@host:22";Port=23}`, then port 23 will be used.</span></span>

<span data-ttu-id="09bae-407">Этот параметр появился в PowerShell 6,0.</span><span class="sxs-lookup"><span data-stu-id="09bae-407">This parameter was introduced in PowerShell 6.0.</span></span>

<span data-ttu-id="09bae-408">Параметр **сшконнектион** полезен для создания нескольких сеансов, в которых каждому сеансу требуются разные сведения о соединении.</span><span class="sxs-lookup"><span data-stu-id="09bae-408">The **SSHConnection** parameter is useful for creating multiple sessions where each session requires different connection information.</span></span>
...
### <span data-ttu-id="09bae-409">-Подсистема</span><span class="sxs-lookup"><span data-stu-id="09bae-409">-Subsystem</span></span>

<span data-ttu-id="09bae-410">Указывает подсистему SSH, используемую для нового **сеанса PSSession** .</span><span class="sxs-lookup"><span data-stu-id="09bae-410">Specifies the SSH subsystem used for the new **PSSession** .</span></span>

<span data-ttu-id="09bae-411">Указывает подсистему для использования в целевом объекте, как определено в sshd_config.</span><span class="sxs-lookup"><span data-stu-id="09bae-411">This specifies the subsystem to use on the target as defined in sshd_config.</span></span>
<span data-ttu-id="09bae-412">Подсистема запускает определенную версию PowerShell с предопределенными параметрами.</span><span class="sxs-lookup"><span data-stu-id="09bae-412">The subsystem starts a specific version of PowerShell with predefined parameters.</span></span>
<span data-ttu-id="09bae-413">Если указанная подсистема не существует на удаленном компьютере, команда завершается ошибкой.</span><span class="sxs-lookup"><span data-stu-id="09bae-413">If the specified subsystem does not exist on the remote computer, the command fails.</span></span>

<span data-ttu-id="09bae-414">Если этот параметр не используется, по умолчанию используется подсистема PowerShell.</span><span class="sxs-lookup"><span data-stu-id="09bae-414">If this parameter is not used, the default is the 'powershell' subsystem.</span></span>
...
### <span data-ttu-id="09bae-415">Общие параметры</span><span class="sxs-lookup"><span data-stu-id="09bae-415">CommonParameters</span></span>

<span data-ttu-id="09bae-416">Этот командлет поддерживает общие параметры: -Debug, -ErrorAction, -ErrorVariable, -InformationAction, -InformationVariable, -OutVariable, -OutBuffer, -PipelineVariable, -Verbose, -WarningAction и -WarningVariable.</span><span class="sxs-lookup"><span data-stu-id="09bae-416">This cmdlet supports the common parameters: -Debug, -ErrorAction, -ErrorVariable, -InformationAction, -InformationVariable, -OutVariable, -OutBuffer, -PipelineVariable, -Verbose, -WarningAction, and -WarningVariable.</span></span> <span data-ttu-id="09bae-417">Дополнительные сведения см. в разделе about_CommonParameters (https://go.microsoft.com/fwlink/?LinkID=113216).</span><span class="sxs-lookup"><span data-stu-id="09bae-417">For more information, see about_CommonParameters (https://go.microsoft.com/fwlink/?LinkID=113216).</span></span>

## <span data-ttu-id="09bae-418">Входные данные</span><span class="sxs-lookup"><span data-stu-id="09bae-418">INPUTS</span></span>

### <span data-ttu-id="09bae-419">System.String, System.URI, System.Management.Automation.Runspaces.PSSession</span><span class="sxs-lookup"><span data-stu-id="09bae-419">System.String, System.URI, System.Management.Automation.Runspaces.PSSession</span></span>

<span data-ttu-id="09bae-420">В этот командлет можно передать строку, URI или объект сеанса.</span><span class="sxs-lookup"><span data-stu-id="09bae-420">You can pipe a string, URI, or session object to this cmdlet.</span></span>

## <span data-ttu-id="09bae-421">Выходные данные</span><span class="sxs-lookup"><span data-stu-id="09bae-421">OUTPUTS</span></span>

### <span data-ttu-id="09bae-422">System.Management.Automation.Runspaces.PSSession</span><span class="sxs-lookup"><span data-stu-id="09bae-422">System.Management.Automation.Runspaces.PSSession</span></span>

## <span data-ttu-id="09bae-423">ПРИМЕЧАНИЯ</span><span class="sxs-lookup"><span data-stu-id="09bae-423">NOTES</span></span>

- <span data-ttu-id="09bae-424">Этот командлет использует инфраструктуру удаленного взаимодействия PowerShell.</span><span class="sxs-lookup"><span data-stu-id="09bae-424">This cmdlet uses the PowerShell remoting infrastructure.</span></span> <span data-ttu-id="09bae-425">Для использования этого командлета локальный компьютер и все удаленные компьютеры должны быть настроены для удаленного взаимодействия PowerShell.</span><span class="sxs-lookup"><span data-stu-id="09bae-425">To use this cmdlet, the local computer and any remote computers must be configured for PowerShell remoting.</span></span> <span data-ttu-id="09bae-426">Дополнительные сведения см. в разделе [about_Remote_Requirements](About/about_Remote_Requirements.md).</span><span class="sxs-lookup"><span data-stu-id="09bae-426">For more information, see [about_Remote_Requirements](About/about_Remote_Requirements.md).</span></span>
- <span data-ttu-id="09bae-427">Чтобы создать **сеанс PSSession** на локальном компьютере, запустите PowerShell с параметром Запуск от имени администратора.</span><span class="sxs-lookup"><span data-stu-id="09bae-427">To create a **PSSession** on the local computer, start PowerShell with the Run as administrator option.</span></span>
- <span data-ttu-id="09bae-428">По завершении работы с **PSSession** используйте `Remove-PSSession` командлет, чтобы удалить **сеанс PSSession** и освободить его ресурсы.</span><span class="sxs-lookup"><span data-stu-id="09bae-428">When you are finished with the **PSSession** , use the `Remove-PSSession` cmdlet to delete the **PSSession** and release its resources.</span></span>
- <span data-ttu-id="09bae-429">Наборы параметров **HostName** и **сшконнектион** были добавлены начиная с PowerShell 6,0.</span><span class="sxs-lookup"><span data-stu-id="09bae-429">The **HostName** and **SSHConnection** parameter sets were included starting with PowerShell 6.0.</span></span>
  <span data-ttu-id="09bae-430">Они были добавлены для обеспечения удаленного взаимодействия PowerShell на основе Secure Shell (SSH).</span><span class="sxs-lookup"><span data-stu-id="09bae-430">They were added to provide PowerShell remoting based on Secure Shell (SSH).</span></span> <span data-ttu-id="09bae-431">SSH и PowerShell поддерживаются на нескольких платформах (Windows, Linux, macOS) и удаленное взаимодействие PowerShell будет работать на этих платформах, где установлены и настроены PowerShell и SSH.</span><span class="sxs-lookup"><span data-stu-id="09bae-431">Both SSH and PowerShell are supported on multiple platforms (Windows, Linux, macOS) and PowerShell remoting will work over these platforms where PowerShell and SSH are installed and configured.</span></span> <span data-ttu-id="09bae-432">Это отдельно от предыдущего удаленного взаимодействия только в Windows, основанного на WinRM, и многие функции и ограничения WinRM не применяются.</span><span class="sxs-lookup"><span data-stu-id="09bae-432">This is separate from the previous Windows only remoting that is based on WinRM and much of the WinRM specific features and limitations do not apply.</span></span> <span data-ttu-id="09bae-433">Например, квоты на основе WinRM, параметры сеанса, конфигурация пользовательской конечной точки и функции отключения и повторного подключения сейчас не поддерживаются.</span><span class="sxs-lookup"><span data-stu-id="09bae-433">For example WinRM based quotas, session options, custom endpoint configuration, and disconnect/reconnect features are currently not supported.</span></span> <span data-ttu-id="09bae-434">Дополнительные сведения о настройке удаленного взаимодействия PowerShell с использованием SSH см. в статье [удаленное взаимодействие PowerShell через SSH](/powershell/scripting/learn/remoting/ssh-remoting-in-powershell-core).</span><span class="sxs-lookup"><span data-stu-id="09bae-434">For more information about how to set up PowerShell SSH remoting, see [PowerShell Remoting Over SSH](/powershell/scripting/learn/remoting/ssh-remoting-in-powershell-core).</span></span>

## <span data-ttu-id="09bae-435">Связанные ссылки</span><span class="sxs-lookup"><span data-stu-id="09bae-435">RELATED LINKS</span></span>

[<span data-ttu-id="09bae-436">Connect-PSSession</span><span class="sxs-lookup"><span data-stu-id="09bae-436">Connect-PSSession</span></span>](Connect-PSSession.md)

[<span data-ttu-id="09bae-437">Disconnect-PSSession</span><span class="sxs-lookup"><span data-stu-id="09bae-437">Disconnect-PSSession</span></span>](Disconnect-PSSession.md)

[<span data-ttu-id="09bae-438">Enter-PSSession</span><span class="sxs-lookup"><span data-stu-id="09bae-438">Enter-PSSession</span></span>](Enter-PSSession.md)

[<span data-ttu-id="09bae-439">Exit-PSSession;</span><span class="sxs-lookup"><span data-stu-id="09bae-439">Exit-PSSession</span></span>](Exit-PSSession.md)

[<span data-ttu-id="09bae-440">Get-PSSession</span><span class="sxs-lookup"><span data-stu-id="09bae-440">Get-PSSession</span></span>](Get-PSSession.md)

[<span data-ttu-id="09bae-441">Invoke-Command</span><span class="sxs-lookup"><span data-stu-id="09bae-441">Invoke-Command</span></span>](Invoke-Command.md)

[<span data-ttu-id="09bae-442">Receive-PSSession</span><span class="sxs-lookup"><span data-stu-id="09bae-442">Receive-PSSession</span></span>](Receive-PSSession.md)

[<span data-ttu-id="09bae-443">Remove-PSSession</span><span class="sxs-lookup"><span data-stu-id="09bae-443">Remove-PSSession</span></span>](Remove-PSSession.md)
