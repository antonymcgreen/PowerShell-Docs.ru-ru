...
## <a name="loading-newly-changed-code-during-development"></a>Загрузка только что измененного кода во время разработки
...
Другой распространенной практикой при разработке является разделение кода на разные файлы. Если функция находится в одном файле, использующем классы, определенные в другом модуле, следует использовать `using module` инструкцию, чтобы убедиться, что функции имеют необходимые определения класса.

## <a name="the-psreference-type-is-not-supported-with-class-members"></a>Тип Псреференце не поддерживается для членов класса

`[ref]`Не удается автоматически использовать приведение типа к члену класса. API-интерфейсы, использующие `[ref]` Параметры, нельзя использовать с членами класса. Класс **псреференце** предназначен для поддержки COM-объектов. Объекты COM содержат случаи, когда необходимо передать значение в по ссылке.
