...
Для поддержки настройки в PSReadLine 2.2.0 были добавлены два параметра цвета `ListView` :

- **Листпредиктионколор** — задает цвет начального `>` символа и конечного имени источника, например `[History]` . По умолчанию используется в `DarkYellow` качестве цвета переднего плана.
- **Листпредиктионселектедколор** — задает цвет, указывающий, что элемент списка выбран. По умолчанию в `DarkBlack` качестве цвета фона используется.
...
- **None** — отключить функцию прогнозной IntelliSense (по умолчанию).
-`**Журнал** . Включите функцию прогнозной IntelliSense и используйте в качестве единственного источника журнал PSReadLine.
- **Подключаемый модуль** . Включите функцию прогнозной IntelliSense и используйте подключаемые модули ( `CommandPrediction` ) в качестве единственного источника. Это значение было добавлено в PSReadLine 2.2.0
- **Хисторяндплугин** . Включите функцию прогнозной IntelliSense и используйте в качестве источников как журнал, так и подключаемый модуль. Это значение было добавлено в PSReadLine 2.2.0
...
Задает стиль для вывода прогнозного текста. Значение по умолчанию — **инлиневиев**.

- **Инлиневиев** — стиль, существующий сегодня, как в оболочке рыбы и ЗШ. (по умолчанию).
- **ListView** — предложения отображаются в раскрывающемся списке, а пользователи могут выбрать использование <kbd>стрелок</kbd> и <kbd>стрелка вниз</kbd>.
