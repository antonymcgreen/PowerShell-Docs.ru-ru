...
Начиная с PowerShell 7, не `Add-Type` компилирует тип, если тип с таким именем уже существует. Кроме того, `Add-Type` ищет сборки в `ref` папке, содержащей `pwsh.dll` .
...
> [!IMPORTANT]
> Начиная с версии PowerShell 7,1 `ConsoleApplication` и `WindowsApplication` не поддерживаются, и PowerShell вызывает завершающую ошибку, если в качестве значений параметра **OutputType** задано значение.
