...
В этом примере используется параметр **ексклудедифферент** для сравнения каждой строки содержимого в двух текстовых файлах.

В PowerShell 7,1 при использовании параметра **ексклудедифферент** выводится **инклудикуал** , а выходные данные содержат только строки, содержащиеся в обоих файлах, как показано в **сидеиндикатор** ( `==` ).
...
Указывает массив свойств **ссылки** и сравниваемых объектов. 
