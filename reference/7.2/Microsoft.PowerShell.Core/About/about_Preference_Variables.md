...
В этом примере показан результат `$DebugPreference` задания значения запроса.  Появится сообщение об отладке, и пользователю будет предложено подтвердить.
...
В этом примере используется параметр **verbose** со значением `$false` , переопределяющим значение запроса  . Пользователю не выводится запрос, и сообщение не отображается.
...
В этом примере `$WarningPreference` переменная изменяется на значение  запроса. Пользователю предлагается подтверждение.
...
Поведение WhatIf можно переопределить с `Stop-Process`  помощью параметра **WhatIf** со значением `$false` . Процесс **Winword** остановлен.
