...
`$input`Переменная также доступна для команды, заданной `-Command` параметром, `pwsh` при вызове из командной строки. Следующий пример выполняется из командной оболочки Windows.
...
Содержит значение `$True` , если текущий сеанс работает в среде выполнения .NET Core (CoreCLR). В противном случае содержит `$False` .

### <a name="islinux"></a>$IsLinux

Содержит `$True` , если текущий сеанс работает в операционной системе Linux.
В противном случае содержит `$False` .

### <a name="ismacos"></a>$IsMacOS

Содержит, `$True` Если текущий сеанс работает в операционной системе MacOS.
В противном случае содержит `$False` .
...
- **PSScriptRoot** — содержит полный путь к скрипту, вызвавшему текущую команду. Значение этого свойства заполняется только в том случае, если вызывающий объект является сценарием.
- **Пскоммандпас** — содержит полный путь и имя файла скрипта, вызвавшего текущую команду. Значение этого свойства заполняется только в том случае, если вызывающий объект является сценарием.
...
Начиная с PowerShell 7, `$PSCulture` отражает язык и региональные параметры текущего пространства выполнения PowerShell (сеанса). Если язык и региональные параметры изменены в пространстве выполнения PowerShell, `$PSCulture` значение этого пространства выполнения обновляется.

Язык и региональные параметры определяют формат отображаемых элементов, таких как числа, денежные единицы и даты, и хранятся в объекте **System. Globalization. CultureInfo** . Используется `Get-Culture` для вывода языка и региональных параметров компьютера. `$PSCulture` содержит значение свойства **Name** .
...
> [!NOTE]
> Эта переменная доступна, только если `PSAnsiRendering` экспериментальная функция IA включена. Дополнительные сведения см. в разделе [about_Experimental_Features](about_Experimental_Features.md) и [использование экспериментальных функций](/powershell/scripting/learn/experimental-features).

Начиная с PowerShell 7,2 теперь можно получить доступ к `$PSStyle` автоматической переменной для просмотра и изменения отрисовки строкового вывода ANSI. Переменная содержит следующие свойства:

- **Сброс** — выключает все украшения
- Объект, вложенный в **фоновый режим** для управления цветом фона
- **Мигающий** — включает мигание
- **Блинкофф** — выключает мерцание
- **Полужирный** — включает полужирный
- **Болдофф** — выключает выделение полужирным шрифтом
- Объект, вложенный в **передний план** , для управления цветом переднего плана
- **Форматирование** — управляет форматированием по умолчанию для выходных потоков
- **Скрыто** — скрывается
- **Хидденофф** — включает скрытые
- **Аутпутрендеринг** — управление при использовании отрисовки выходных данных
- **Обратная** переключается на обратный
- **Реверсеофф** — отключает обратный
- **Курсив** — включает курсив
- **Италикофф** — выключает курсив
- **Подчеркнутый — включает подчеркивание**
- **Ундерлинеофф** — выключает подчеркивание

Базовые элементы возвращают строки escape-последовательностей ANSI, сопоставленные с их именами.
Значения можно настраивать. Например, можно изменить полужирный шрифт на подчеркнутый. Имена свойств упрощают создание декорированных строк с помощью функции завершения по клавише TAB:

```powershell
"$($PSStyle.Background.LightCyan)Power$($PSStyle.Underline)$($PSStyle.Bold)Shell$($PSStyle.Reset)"
```

`$PSStyle.Background`Элементы и `$PSStyle.Foreground` являются строками, содержащими escape-последовательности ANSI для 16 стандартных цветов консоли, а также `Rgb()` метод для указания формата 24 бит. Значения являются устанавливаемыми и могут содержать любое количество escape-последовательностей ANSI.

`$PSStyle.Formatting` — Это вложенный объект для управления форматированием по умолчанию в сообщениях об отладке, ошибке, подробностях и предупреждениях. Можно также управлять такими атрибутами, как полужирный и подчеркивание. Он заменяется `$Host.PrivateData` способом управления цветами для отрисовки форматирования. `$Host.PrivateData` сохраняется для обеспечения обратной совместимости, но не соединено с `$PSStyle.Formatting` .

`$PSStyle.OutputRendering` является `System.Management.Automation.OutputRendering` перечислением со значениями:

- **Автоматически**. это значение по умолчанию. Если узел поддерживает Виртуалтерминал, то ANSI всегда передается как есть, в противном случае — в виде открытого текста
- **ANSI**: ANSI всегда передается как есть
- **Обычный** текст: escape-последовательности ANSI всегда удаляются, чтобы они были только обычным текстом.
- **HostOnly**: это поведение macOS, при котором escape-последовательности ANSI удаляются в перенаправленном или перенаправляемом выводе.
...
- **PSEdition** — это свойство имеет значение "Desktop" для PowerShell 4 и ниже, а также PowerShell 5,1 в полнофункциональных выпусках Windows. Это свойство имеет значение "Core" для PowerShell 6 и более поздних версий, а также PowerShell PowerShell 5,1 в выпусках с меньшим объемом, таких как Windows Nano Server или Windows IoT.
- **Гиткоммитид** — идентификаторы фиксации исходных файлов в GitHub;
- **OS** — Описание операционной системы, на которой работает PowerShell.
- **Платформа — Платформа** , на которой работает операционная система. Значение в Linux и macOS — **UNIX**. См. разделы `$IsMacOs` и `$IsLinux`.
- **Пскомпатиблеверсионс** версии PowerShell, совместимые с текущей версией
- **Псремотингпротоколверсион** — версия протокола удаленного управления PowerShell.
