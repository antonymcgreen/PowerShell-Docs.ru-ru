...
В PowerShell 7,1 в системах UNIX `Get-Item` командлет предоставляет выходные данные, подобные UNIX:
...
Возвращает указанный альтернативный поток данных из файла. Введите имя потока. Поддерживаются подстановочные знаки. Чтобы получить все потоки, используйте звездочку ( `*` ). Этот параметр является допустимым для каталогов, но обратите внимание, что по умолчанию каталоги не имеют потоков данных.

Этот параметр появился в PowerShell 3,0.  Начиная с PowerShell 7,2, `Get-Item` может получать альтернативные потоки данных из каталогов, а также файлов.
