...
Возвращает объект, представляющий элемент, с которым вы работаете. По умолчанию этот командлет не создает выходные данные. Поддержка параметра **PassThru** была добавлена в **PowerShellGet** 2.0.0
...
- В PowerShellGet версий 2.0.0 и выше значение по умолчанию — **ALLUSERS** при запуске сеанса с повышенными правами и **CurrentUser** для всех остальных.
...
Для PowerShell 5,1 или ниже областью по умолчанию в сеансе с повышенными правами является **ALLUSERS** и в сеансе без повышенных привилегий **CurrentUser** . Обновления модулей для **ALLUSERS** , `$env:ProgramFiles\PowerShell\Modules` требуют повышенных разрешений. Обновления модулей для **CurrentUser** , `$home\Documents\PowerShell\Modules` не требуют повышенных разрешений.
