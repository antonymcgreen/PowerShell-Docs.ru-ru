...
### Пример 1. изменение состояния подразделения потока

```
PS C:\> Set-PSSessionConfiguration -Name "MaintenanceShell" -ThreadApartmentState STA
```

Эта команда изменяет состояние модели "apartment" потока в конфигурации MaintenanceShell на STA.
Изменение вступает в силу при перезапуске службы **WinRM** .

### Пример 2. Создание и изменение конфигурации сеанса
...
### Пример 3. Отображение результатов
...
### Пример 4. Отображение результатов различными способами
...
Задает параметр состояния апартамента для потоков в сеансе.
