...
- Пустой. По умолчанию модули не добавляются в сеанс. Параметры этого командлета позволяют добавить в сеанс модули, функции, скрипты и другие элементы.
- По умолчанию. Добавляет Microsoft. PowerShell. Core в сеанс. Этот модуль включает `Import-Module` командлет, который пользователи могут использовать для импорта других модулей, если только командлет не запрещен явным образом.
- RestrictedRemoteServer. Включает только следующие командлеты: `Exit-PSSession` , `Get-Command` , `Get-FormatData` ,,, `Get-Help` `Measure-Object` `Out-Default` и `Select-Object` . Скрипт, сборку и ключи в файле конфигурации сеанса для добавления позволяют добавлять в сеанс модули, функции, скрипты и другие элементы.
...
Указывает состояние апартамента потоков в сеансе.
Допустимые значения для этого параметра: STA, MTA и Unknown.
