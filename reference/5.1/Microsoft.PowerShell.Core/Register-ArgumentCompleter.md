...
В блоке скрипта первая команда извлекает все работающие службы с помощью `Where-Object` командлета. Службы передаются в `ForEach-Object` командлет. `ForEach-Object`Командлет создает новый [`[System.Management.Automation.CompletionResult]`](/dotnet/api/system.management.automation.completionresult) объект и заполняет его значениями текущей службы (представленной переменной конвейера `$_` ).
