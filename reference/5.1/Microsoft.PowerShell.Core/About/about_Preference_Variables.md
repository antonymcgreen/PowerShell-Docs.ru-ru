...
- **Нормалвиев** : (по умолчанию) подробное представление, предназначенное для большинства пользователей. Состоит из описания ошибки и имени объекта, участвующего в ошибке.
...
В этом примере показано, как возникает ошибка, если значением `$ErrorView` по умолчанию является **нормалвиев**. `Get-ChildItem` используется для поиска несуществующего файла.
...
- **Приостановить** : автоматически приостанавливает задание рабочего процесса после выполнения `Write-Information` команды, чтобы разрешить пользователям видеть эти сообщения перед тем, как продолжить. Рабочий процесс можно возобновить по собственному усмотрению пользователя.
...
### <a name="maximumaliascount"></a>\$максимумалиаскаунт

Определяет, сколько псевдонимов разрешено в сеансе PowerShell. Значение по умолчанию — **4096** , которое должно быть достаточным для большинства случаев использования. Можно настроить `$MaximumAliasCount` в соответствии с вашими потребностями.
...
### <a name="maximumdrivecount"></a>\$максимумдривекаунт

Определяет, сколько дисков PowerShell разрешено в данном сеансе. Например, диски файловой системы и хранилища данных, предоставляемые поставщиками PowerShell и отображаемые в виде дисков, таких как `Alias:` `HKLM:` диски и.
...
Объекты, представляющие каждую сохраненную ошибку, хранятся в `$Error` автоматической переменной. `$Error` содержит массив объектов записей об ошибках. Самая последняя ошибка — первый объект в массиве, `$Error[0]` .

Чтобы подсчитать количество ошибок в системе, используйте `$Error` свойство **Count** массива.

```powershell
$Error.count
```

Чтобы отобразить конкретную ошибку, используйте `[0]` нотацию массива для просмотра самой последней ошибки.

```powershell
$Error[0]
```

Чтобы отобразить самую старую ошибочную ошибку, введите:

```powershell
$Error[($Error.Count -1]
```

Параметр **Force** переопределяет особое форматирование объектов **ерроррекорд** и возвращается к обычному формату. Чтобы отобразить свойства объекта **ерроррекорд** , введите следующую команду:

```powershell
$Error[0] | Format-List -Property * -Force
```

В этом примере `$Error.Count` выводится количество ошибок. Чтобы удалить все ошибки из журнала ошибок, используйте `Clear` метод массива ошибок.
...
Чтобы найти все свойства и методы массива ошибок, используйте `Get-Member` командлет со своим параметром **InputObject** . При использовании параметра **InputObject** `Get-Member` отображает свойства и методы коллекции.

```powershell
Get-Member -InputObject $Error
```

При передаче коллекции объектов в `Get-Member` `Get-Member` отображается список свойств и методов объектов в коллекции.

```powershell
$Error | Get-Member
```

### <a name="maximumfunctioncount"></a>\$максимумфунктионкаунт
...
Чтобы просмотреть функции в сеансе, используйте `Function:` диск PowerShell, предоставляемый `Function` поставщиком PowerShell. Дополнительные сведения о `Function` поставщике [about_Function_Provider](about_Function_Provider.md).

Чтобы вывести список функций в текущем сеансе, введите:

```powershell
Get-ChildItem Function:
```

Чтобы подсчитать количество функций в текущем сеансе, введите:
...
### <a name="maximumvariablecount"></a>\$максимумвариаблекаунт

Определяет, сколько переменных разрешено в данном сеансе, включая автоматические переменные, привилегированные переменные и переменные, создаваемые в командах и скриптах.

**Допустимые значения** : 1024-32768 ( `Int32` )

**По умолчанию** : 4096

Чтобы просмотреть переменные в сеансе, используйте `Get-Variable` командлет и функции `Variable:` диска PowerShell и `Variable` поставщика PowerShell. Дополнительные сведения см. в разделе [about_Variable_Provider](about_Variable_Provider.md).

Чтобы найти текущее число переменных в системе, введите:
