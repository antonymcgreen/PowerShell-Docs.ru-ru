...
Кроме того, в строке можно ссылаться на захваченные группы \<substitute\> .
Это делается с помощью `$` символа перед идентификатором группы.

Два способа ссылки на группы захвата — **число** и **имя**

- По **числу** -
   захваченных групп нумеруются слева направо.
...
- По **имени** -
   захваченные группы также могут ссылаться по имени.
...
> [!WARNING]
> Поскольку `$` символ используется в расширении строки, необходимо использовать литеральные строки с подстановкой или Escape- `$` символ.
>
> ```powershell
...
> ```
>
> Кроме того, поскольку `$` символ используется в подстановке, необходимо будет экранировать все экземпляры в строке.
>
> ```powershell
> '5.72' -replace '(.+)', '$$$1'
> ```
