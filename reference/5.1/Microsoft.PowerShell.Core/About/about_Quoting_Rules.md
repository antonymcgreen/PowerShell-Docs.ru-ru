...
Кавычки также используются для создания _строки String_. Строка Here — это строка, заключенная в одинарные кавычки или двойные кавычки, в которой кавычки обрабатываются буквально. Строка here может охватывать несколько строк. Все строки в этой строке считаются строками, даже если они не заключены в кавычки.
...
Строка, заключенная в двойные кавычки, является _расширяемой_ строкой. Имена переменных, начинающиеся со знака доллара ( `$` ), заменяются значением переменной перед передачей строки в команду для обработки.
...
Строка, заключенная в одинарные кавычки, представляет собой _буквальную_ строку. Строка передается в команду точно так же, как при ее вводе. Подстановка не выполняется.
Пример:
