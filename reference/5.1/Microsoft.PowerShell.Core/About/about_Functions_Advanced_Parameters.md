...
Существует известная ошибка при использовании коллекций с **валуефромремаинингаргументс** , где переданная коллекция рассматривается как один элемент.

В следующем примере показана эта известная проблема. **Оставшийся** параметр должен содержать **один** с **индексом 0** и **двумя** с **индексом 1**.
Вместо этого оба элемента объединяются в одну сущность.
...
> [!NOTE]
> Эта проблема решена в PowerShell 6,2.
...
Атрибут **валидатеранже** задает числовой диапазон для каждого значения параметра или переменной. PowerShell выдает ошибку, если какое-либо значение выходит за пределы этого диапазона. В следующем примере значение параметра **попыток** должно находиться в диапазоне от 0 до десяти.
