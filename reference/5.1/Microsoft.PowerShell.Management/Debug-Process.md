...
Команда использует `Get-Process` командлет для получения процессов MyApp на компьютерах Server01 и Server02. Он использует оператор конвейера для отправки процессов в `Debug-Process` командлет, который прикрепляет отладчики.
