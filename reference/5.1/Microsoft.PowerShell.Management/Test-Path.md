...
Если для параметра указана пустая строка `-Path` , возвращается **значение true** . Если указана пустая строка, функция `Test-Path` возвращает ошибку. В следующем примере показаны пробелы и пустая строка.
...
Объект `Test-Path` предназначен для работы с данными, предоставляемыми любым поставщиком.
