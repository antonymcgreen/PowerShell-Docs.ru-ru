...
С помощью этого командлета можно добавлять события в каналы ETW из Windows PowerShell.
...
Если указанные полезные данные не соответствуют полезным данным в определении события, среда Windows PowerShell создает предупреждение, но команда завершается успешно.
...
Указывает номер версии события. Введите номер события. Среда Windows PowerShell преобразует номер в требуемый тип Byte.
