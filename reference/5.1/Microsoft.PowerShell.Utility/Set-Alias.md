...
Чтобы создать новый псевдоним, используйте `Set-Alias` или `New-Alias` . Чтобы удалить псевдоним, используйте `Remove-Item` командлет. Например, `Remove-Item -Path Alias:aliasname`.
