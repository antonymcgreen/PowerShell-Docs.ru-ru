...
Используйте команду **Import-PSSession** , чтобы импортировать команды из настроенной оболочки, например оболочки Microsoft Exchange Server, или из сеанса Windows PowerShell, включающего в себя модули и оснастки или другие элементы, которые находятся не в текущем сеансе.
...
Однако удаленные операции полностью обрабатываются Windows PowerShell.
...
Этот параметр позволяет выполнить команду в качестве фонового задания Windows PowerShell.
Дополнительные сведения см. в разделе about_Jobs.

При использовании **Import-PSSession** Windows PowerShell добавляет импортированные команды во временный модуль, который существует только в текущем сеансе, и возвращает объект, представляющий этот модуль.
Чтобы создать постоянный модуль, который можно использовать в будущих сеансах, используйте командлет Export-PSSession.

Командлет **Import-PSSession** использует функцию неявного удаленного взаимодействия Windows PowerShell.
...
В общем случае этот модуль добавляется во все сеансы с помощью команды **Import-Module** в профиле Windows PowerShell, однако профили не работают в сеансах PSSession.
...
Поскольку функции имеют приоритет над командлетами, Windows PowerShell выполняет импортированную функцию **Get-Date** , которая возвращает дату по юлианскому календарю.
...
Четвертая команда возвращает имя оснастки Windows PowerShell, добавившей исходный командлет **Get-Date** в текущий сеанс.
...
Однако при передаче объекта модуля Windows PowerShell использует для него метод **ToString** , который возвращает имя модуля.
...
- Псевдоним.
Псевдонимы Windows PowerShell в удаленном сеансе.
- Все.
Командлеты и функции в удаленном сеансе.
- приложение.
Все файлы, кроме файлов Windows-PowerShell в путях, перечисленных в переменной среды Path ($env:путь) в удаленном сеансе, включая файлы TXT, EXE и DLL.
- Командлет.
...
- Фильтр и функция.
Функции Windows PowerShell в удаленном сеансе.
- Скрипт.
...
### -Module
Указывает и массив команд в оснастках и модулях Windows PowerShell.
...
* **Import-PSSession** основывается на инфраструктуре удаленного взаимодействия Windows PowerShell. Для использования этого командлета компьютер должен быть настроен на удаленное взаимодействие WS-Management. Дополнительные сведения см. в разделе about_Remote и about_Remote_Requirements.
* **Import-PSSession** не импортирует переменные или поставщики Windows PowerShell.
...
* Поскольку профили Windows PowerShell не выполняются в сеансах PSSession, команды, добавляемые профилем в сеанса, недоступны для **Import-PSSession**. Для импорта команд из профиля используйте команду Invoke-Command, чтобы вручную запустить профиль в PSSession перед импортом команд.
