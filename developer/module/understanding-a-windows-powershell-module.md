...
- Динамические модули, создаваемые `New-Module` командлетом, могут называться `Name` с помощью параметра командлета.

- Модули, импортированные из объектов сборки с помощью `"dynamic_code_module_" + assembly.GetName()`  **`Import-Module` команды-Assembly** , именуются в соответствии со следующим синтаксисом:.
...
Командлет [Export-ModuleMember](/powershell/module/Microsoft.PowerShell.Core/Export-ModuleMember) . Этот командлет задает элементы модуля (например, командлеты, функции, переменные и псевдонимы), которые экспортируются из файла модуля скрипта (. PSM1) или из динамического модуля, созданного `New-Module` с помощью командлета.
