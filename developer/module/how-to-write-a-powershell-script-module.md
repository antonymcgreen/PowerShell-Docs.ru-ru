...
1. Использовать существующий скрипт PowerShell и сохраните сценарий с расширением .psm1.
   ...
   Пример сценария PowerShell, имеют право Show-Calendar, находится в конце этого раздела.
...
2. Если вы хотите управлять доступом пользователей к определенных функций или свойств, вызовите [Export-ModuleMember](/powershell/module/Microsoft.PowerShell.Core/Export-ModuleMember) в конце скрипта.
...
4. Если нужно описать вашего модуля в PowerShell справочную систему, это можно сделать либо с комментариями Стандартная Справка в этом файле, либо с помощью файла справки.

   Пример кода в конце этого раздела включает справочные сведения в комментариях. Если это необходимо, можно также написать извлеченные файлы XML, содержащие дополнительное содержимое справки. Дополнительные сведения см. в разделе [написание справки для Windows PowerShell модулей](./writing-help-for-windows-powershell-modules.md).
...
Ниже приведен простой сценарий модуль, содержащий одну функцию с именем Show-Calendar. Эта функция отображает визуальное представление календаря. Кроме того пример содержит строки справки PowerShell краткий обзор, описание и значения параметров, а также пример. Обратите внимание на то, что последняя строка кода указывает, что функция Show-Calendar экспортируется как член модуля при импорте модуля.
