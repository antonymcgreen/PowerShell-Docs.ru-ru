...
Пользователь может просматривать ошибки на основе их категории, присвоив `$ErrorView` переменной значение "категоривиев".
...
Это можно сделать, определив `ErrorAction` параметры и. `ErrorVariable`
При определении `ErrorAction` параметра командлет представляет пользовательский параметр [System. Management. Automation. действие][]. Вы также можете напрямую повлиять на `$ErrorActionPreference` действие, задав переменную.

Командлет может сохранять неустранимые ошибки в переменную с помощью `ErrorVariable` параметра, который не зависит от `ErrorAction`значения параметра.
