...
В этом примере показано, как для поддержки задания при написании командлетов. Если требуется, чтобы пользователям запускать командлет как фоновое задание, необходимо включить код, описанный в следующей процедуре. Дополнительные сведения о фоновых заданиях см. в разделе [фоновые задания](./background-jobs.md).

## <a name="to-support-jobs"></a>Для поддержки заданий

1. Определение `AsJob` параметр-переключатель, чтобы пользователь может решить, следует ли выполнить командлет, как задание.

    В следующем примере объявление параметра AsJob.
...
3. Добавьте в метод обработки записей, `if` инструкцию, чтобы определить, следует ли запускать командлет, как задание. В следующем коде используется [System.Management.Automation.Cmdlet.Processrecord*](/dotnet/api/System.Management.Automation.Cmdlet.ProcessRecord) метод.
...
4. Для задания пользовательских объектов Реализуйте класс задания.
...
5. Если командлет выполняет работу, вызвать [System.Management.Automation.Cmdlet.Writeobject*](/dotnet/api/System.Management.Automation.Cmdlet.WriteObject) метод вернет объект процесса в конвейер. Если работа выполняется как задание, добавьте дочернего задания к заданию.
...
В следующем примере кода показан код для **Get-Proc** командлет, который можно получить процессы внутренним образом или с помощью фонового задания.
