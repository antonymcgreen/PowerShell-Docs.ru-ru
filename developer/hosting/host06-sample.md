...
В этом примере показано, как сборку интерактивного консольного ведущего приложения, читает команды из командной строки, выполняет команды и затем отображает результаты на консоль. Кроме того, в этом примере используются интерфейсы API создателя токенов для указания цвета текста, вводимого пользователем.

## <a name="requirements"></a>Требования

- В этом примере требуется Windows PowerShell 2.0.

- Это приложение должно выполняться в режиме с повышенными правами (Запуск от имени администратора).
...
- Для создания консольного приложения, использует эти классы узла для создания интерактивной оболочки Windows PowerShell.

- Создание `$profile` переменной и загрузке следующие профили.
...
- Используйте API маркировки для выделения цветом командной строки, как оно введено.

## <a name="example"></a>Пример

 В этом примере пользователь может вводить команды в командной строке, обрабатывает эти команды и затем выводит результаты.
...
## <a name="example"></a>Пример
 ...
 Следующий код является реализацией [System.Management.Automation.Host.PSHost](/dotnet/api/System.Management.Automation.Host.PSHost) класс, используемый ведущим приложением. Те элементы, которые не реализованы исключение или не возвращать ничего.
...
## <a name="example"></a>Пример
 ...
 Следующий код является реализацией [System.Management.Automation.Host.Pshostuserinterface](/dotnet/api/System.Management.Automation.Host.PSHostUserInterface) класс, используемый ведущим приложением.
...
## <a name="example"></a>Пример
 ...
 Следующий код является реализацией [System.Management.Automation.Host.Pshostrawuserinterface](/dotnet/api/System.Management.Automation.Host.PSHostRawUserInterface) класс, используемый ведущим приложением. Те элементы, которые не реализованы исключение или не возвращать ничего.
...
## <a name="example"></a>Пример
 ...
 Следующий код считывает командную строку и цвета текста при его вводе. Токены определяются с помощью [System.Management.Automation.Psparser.Tokenize*](/dotnet/api/System.Management.Automation.PSParser.Tokenize) метод.
