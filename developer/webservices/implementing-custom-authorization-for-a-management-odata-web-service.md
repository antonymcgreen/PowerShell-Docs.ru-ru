---
title: Авторизация на настраиваемый веб-службы OData для управления | Документация Майкрософт
ms.custom: ''
ms.date: 09/13/2016
ms.reviewer: ''
ms.suite: ''
...
ms.locfileid: "58057898"
---
# <a name="implementing-custom-authorization-for-a-management-odata-web-service"></a><span data-ttu-id="fd824-102">Реализация пользовательской авторизации для веб-службы управления OData</span><span class="sxs-lookup"><span data-stu-id="fd824-102">Implementing Custom Authorization for a Management OData web service</span></span>

<span data-ttu-id="fd824-103">Использование веб-службы Windows PowerShell требует третьих лиц для реализации [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) интерфейс для предоставления командлетов Windows PowerShell.</span><span class="sxs-lookup"><span data-stu-id="fd824-103">Using the Windows PowerShell Web Service requires a third party to implement the [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) interface to expose Windows PowerShell cmdlets.</span></span> <span data-ttu-id="fd824-104">Этот интерфейс выполняет авторизацию пользователя для веб-службы.</span><span class="sxs-lookup"><span data-stu-id="fd824-104">This interface performs user authorization to the web service.</span></span> <span data-ttu-id="fd824-105">После написания кода для реализации интерфейса, необходимо скомпилировать его в библиотеку DLL для использования в веб-приложения.</span><span class="sxs-lookup"><span data-stu-id="fd824-105">After writing the code to implement the interface, you must compile it into a DLL to be used in the web application.</span></span>

## <a name="pass-through-authorization"></a><span data-ttu-id="fd824-106">Сквозной авторизации</span><span class="sxs-lookup"><span data-stu-id="fd824-106">Pass-through authorization</span></span>

<span data-ttu-id="fd824-107">Самый простой способ реализации [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) интерфейс является транзитной реализацией, которая разрешает всем пользователям.</span><span class="sxs-lookup"><span data-stu-id="fd824-107">The simplest way to implement the [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) interface is a pass-through implementation that authorizes all users.</span></span> <span data-ttu-id="fd824-108">Этот пример содержит без безопасности, а также s, предоставляются только в качестве показано, как реализовать интерфейс.</span><span class="sxs-lookup"><span data-stu-id="fd824-108">This example provides no security, and s provided only as an illustration of how to implement the interface.</span></span> <span data-ttu-id="fd824-109">Реализация [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) интерфейс необходимо переопределить два метода: [Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) и [Microsoft.Management.Odata.CustomAuthorization.GetMembershipId](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.GetMembershipId).</span><span class="sxs-lookup"><span data-stu-id="fd824-109">An implementation of the  [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) interface must override two methods: [Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) and [Microsoft.Management.Odata.CustomAuthorization.GetMembershipId](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.GetMembershipId).</span></span> <span data-ttu-id="fd824-110">В этом примере [Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) всегда возвращает **System.Security.Principal.WindowsIdentity** объект, связанный с текущим пользователем.</span><span class="sxs-lookup"><span data-stu-id="fd824-110">In this example, the [Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) always returns the **System.Security.Principal.WindowsIdentity** object associated with the current user.</span></span>
...
### <a name="role-based-authorization"></a><span data-ttu-id="fd824-111">Авторизация на основе ролей</span><span class="sxs-lookup"><span data-stu-id="fd824-111">Role-based authorization</span></span>

<span data-ttu-id="fd824-112">В следующем примере реализуется политика авторизации на основе ролей.</span><span class="sxs-lookup"><span data-stu-id="fd824-112">The following example implements a role-based authorization policy.</span></span> <span data-ttu-id="fd824-113">Политика определяется в XML-файл, который находится в каталоге основного приложения с помощью файла web.config и MOF и XML-файлы схемы сопоставления.</span><span class="sxs-lookup"><span data-stu-id="fd824-113">The policy is defined in an XML file that resides in the main application directory with the web.config and MOF and XML mapping schema files.</span></span> <span data-ttu-id="fd824-114">Сведения о настройке файла схемы авторизации, см. в разделе [авторизации на основе ролей, Настройка](./configuring-role-based-authorization.md).</span><span class="sxs-lookup"><span data-stu-id="fd824-114">For information about how to configure the authorization schema file, see [Configuring Role-based Authorization](./configuring-role-based-authorization.md).</span></span> <span data-ttu-id="fd824-115">В первой части примера реализует [Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) и [Microsoft.Management.Odata.CustomAuthorization.GetMembershipId](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.GetMembershipId) методы.</span><span class="sxs-lookup"><span data-stu-id="fd824-115">The first part of the sample implements the [Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) and [Microsoft.Management.Odata.CustomAuthorization.GetMembershipId](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.GetMembershipId) methods.</span></span> <span data-ttu-id="fd824-116">В этом случае методы интерфейса вызывать методы в `RbacSystem` класс (как определено ниже), что делать фактическую работу по проверке разрешений для пользователя.</span><span class="sxs-lookup"><span data-stu-id="fd824-116">In this case, the interface methods call methods in the `RbacSystem` class (defined below) that do the actual work of checking the permissions for the user.</span></span>
...
<span data-ttu-id="fd824-117">В следующем примере создается класс, который выполняет синтаксический анализ политики авторизации в XML-файле.</span><span class="sxs-lookup"><span data-stu-id="fd824-117">The following example creates a class that parses authorization policy in the XML file.</span></span>
...
```
 ...
 <span data-ttu-id="fd824-118">В следующих классах представлены групп и пользователей, соответственно.</span><span class="sxs-lookup"><span data-stu-id="fd824-118">The following classes represent groups and users, respectively.</span></span>
...
<span data-ttu-id="fd824-119">Наконец, класс RbacSystem реализует методы, которые выполняют проверки разрешений для пользователя и возвращено состояние авторизации методы, определенные в реализации [Microsoft.Management.Odata.CustomAuthorization ](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) интерфейс.</span><span class="sxs-lookup"><span data-stu-id="fd824-119">Finally, the RbacSystem class implements methods that do the work of checking the permissions for the user and return the authorization status to the methods defined in the implementation of the [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) interface.</span></span>