---
title: Создание схемы MOF-файл для веб-службы OData для управления | Документация Майкрософт
ms.custom: ''
ms.date: 09/13/2016
ms.reviewer: ''
ms.suite: ''
...
Определяются ресурсы, которые предоставляет веб-службу OData для управления, создав файл MOF, используемые схемы общедоступного ресурса. Каждый ресурс определен как класс в файле, а свойства определяются в виде членов класса. Дополнительные сведения о схеме, используемой в MOF-файл, см. в разделе [общедоступной схемы ресурсов](./public-resource-schema.md).

## <a name="example-mof-file"></a>Пример файла MOF

Следующий файл определяет службы и обработки ресурсов. Каждый из этих ресурсов соответствует объекту, можно управлять с помощью набора командлетов Windows PowerShell. Свойства соответствуют параметры, используемые этими командлетами.

Каждый из двух ресурсов содержит свойства сложного типа. Сложные типы определяются как измененные классы с `ComplexType` квалификатор.
